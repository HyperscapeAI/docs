...
├── ui/             # Pure design system (hooks, stores, base components)
│   ├── components/ # Reusable UI primitives (Window, TabBar, DragOverlay, etc.)
│   ├── core/       # Core UI systems (drag, edit, notifications, presets, responsive, tabs, tooltip, virtual, window)
│   ├── stores/     # UI state management (windowStore, dragStore, editStore, etc.)
...
│   ├── panels/     # Game panels (inventory, equipment, combat, skills, etc.)
...
<Info>
  The architecture makes clear decisions: reusable UI primitives go in `ui/`, game-specific code goes in `game/`.
</Info>
