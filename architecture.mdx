...
| `model-bounds.json` | Auto-generated model bounding boxes and footprints |
...
<Info>
**Automatic Footprint Detection**: The `model-bounds.json` manifest is auto-generated at build time by scanning GLB files. Station footprints are calculated from `(model dimensions × scale) / tile size`.
</Info>
