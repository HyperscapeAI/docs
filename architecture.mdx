...
**Key Features:**
- Frontend served from Cloudflare's global CDN (low latency)
- Server handles game logic and WebSocket connections
- Assets served from R2 (not bundled with frontend)
- Manifests fetched from CDN at server startup (allows content updates without redeployment)
...
Hyperscape uses a **split deployment model** for optimal performance and cost:

```mermaid
flowchart LR
    A[Client Browser] -->|HTTPS| B[Cloudflare Pages]
    A -->|WSS/HTTPS| C[Railway Server]
    C -->|Fetch| D[Cloudflare R2 CDN]
    C -->|Query| E[Railway PostgreSQL]
    B -->|Assets| D
```
...
|---------|----------|---------|
| **Frontend** | Cloudflare Pages | Static site hosting (`hyperscape.club`) |
| **Backend/API** | Railway | Game server with WebSockets |
| **Database** | Railway PostgreSQL | Player data persistence |
| **Assets/CDN** | Cloudflare R2 | 3D models, audio, textures |

### Deployment Flow

1. **Push to main** triggers GitHub Actions
2. **Railway workflow** builds server + client via Nixpacks
3. **Client build** copied to `packages/server/public/`
4. **Server starts**, fetches manifests from CDN
5. **Unified deployment** serves both API and frontend

<Info>
The server serves the built client from `/public/` for unified deployment, while Cloudflare Pages hosts the primary frontend for better global distribution.
</Info>
