...
flowchart LR
    ...
    B -->|Assets| D[Cloudflare R2 CDN]
    C -->|Manifests| D
    C -->|Data| E[PostgreSQL]
    
    B[Frontend<br/>hyperscape.club]
    C[Backend/API<br/>Railway]
    D[Assets/CDN<br/>R2]
    E[Database<br/>Railway/Neon]
```

**Components:**
- **Frontend**: Static React app on Cloudflare Pages with global edge caching
- **Backend**: Game server on Railway with WebSocket support
- **CDN**: Cloudflare R2 for 3D models, audio, textures, and manifests
- **Database**: PostgreSQL on Railway or Neon for player data persistence

**Benefits:**
- Global edge caching for static assets
- Low-latency WebSocket connections to game server
