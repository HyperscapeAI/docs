...
The shared package includes a centralized particle system that uses GPU instancing for high-performance visual effects:

**Architecture:**
- **ParticleManager**: Central router that dispatches particle events to specialized sub-managers
- **WaterParticleManager**: GPU-instanced fishing spot effects (4 InstancedMeshes with TSL shaders)
...
- GPU-driven animations (parabolic arcs, wobble, twinkle, ring expansion)
...
- **Fishing Variants**: Net (calm), Bait (medium), Fly (active) with different particle counts
- **Burst System**: Fish activity bursts fire 2-4 splash particles every 3-7 seconds

**Integration:**
- ResourceSystem creates ParticleManager on client startup
- ResourceEntity delegates particle lifecycle to ParticleManager
- Per-frame update driven by ResourceSystem.update()
- Lazy registration pattern (retries if manager not ready during entity init)
- Proper cleanup via unregister on entity destroy

**Extensibility:**
To add new particle types (fire, magic, dust):
...
See [Particle System Documentation](/wiki/engine/particles) for complete API reference.
