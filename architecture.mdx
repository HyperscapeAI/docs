...
└── Dockerfile.server       # Production Docker build
```

## Production Architecture

Hyperscape uses a distributed deployment model:
...
│  │  (Edge Caching)  │         │   (CDN/Storage)  │      │
│  │                  │         │                  │      │
│  │ hyperscape.club  │         │ assets.hyperscape│      │
│  │                  │         │      .club       │      │
│  └────────┬─────────┘         └──────────────────┘      │
│           │                                              │
└───────────┼──────────────────────────────────────────────┘
...
│  │  (Fastify + WS)  │         │   (Persistent)   │      │
│  │                  │         │                  │      │
│  │ hyperscape-prod  │         │  Player data     │      │
│  │ .up.railway.app  │         │  World state     │      │
│  └──────────────────┘         └──────────────────┘      │
│                                                           │
└───────────────────────────────────────────────────────────┘
```
...
**Traffic Flow**:
1. Users access `hyperscape.club` (Cloudflare Pages)
2. Client loads from edge network (global CDN)
3. Client connects to Railway server via WebSocket
4. Assets load from `assets.hyperscape.club` (Cloudflare R2)
5. Server queries PostgreSQL for player data

**Benefits**:
- **Low latency**: Frontend served from nearest edge location
- **Scalability**: CDN handles static asset traffic
- **Reliability**: Cloudflare's global network
- **Cost-effective**: Railway free tier for server, Cloudflare free tier for CDN
