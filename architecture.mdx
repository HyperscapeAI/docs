...
Hyperia is a **Turbo monorepo** with core packages:

```
hyperforge/
├── packages/
│   ├── shared/              # @hyperia/shared - Core 3D engine (ECS, Three.js, PhysX, React UI)
│   ├── server/              # @hyperia/server - Game server (Fastify, WebSockets, PostgreSQL)
│   ├── client/              # @hyperia/client - Web client (Vite, React)
│   ├── plugin-hyperia/      # @hyperia/plugin-hyperia - ElizaOS AI agent plugin
│   ├── physx-js-webidl/     # @hyperia/physx-js-webidl - PhysX WASM bindings
│   ├── procgen/             # @hyperia/procgen - Procedural generation (trees, terrain, buildings)
...
│   └── website/             # @hyperia/website - Marketing website (Next.js 15)
...
### shared (`@hyperia/shared`)

The core Hyperia 3D engine containing:
...
<Warning>
**WebGPU Required**: All rendering uses TSL (Three Shading Language) which only works with WebGPU. WebGL fallback was removed in February 2026. Requires Chrome 113+, Edge 113+, or Safari 18+ (macOS 15+). **Note:** Safari 17 support was removed - Safari 18+ (macOS 15+) is now required.
</Warning>
...
### plugin-hyperia (`@hyperia/plugin-hyperia`)
...
```typescript
// From packages/plugin-hyperia/package.json (as of March 2026)
...
- **Previous Issue**: `@hyperia/shared` imports from `@hyperia/procgen`, procgen imports from shared
...
**plugin-hyperia ↔ shared circular dependency:**
