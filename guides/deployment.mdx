...
**GPU Rendering Requirements (commits e51a332, 30bdaf0, Feb 27 2026):**

The system requires hardware GPU rendering for WebGPU. Two modes are supported:
...
2. **Headless EGL Mode** (fallback for containers without DRM):
   - Works without X server or DRM/DRI access
   - Uses Chrome's new headless mode with EGL
   - Hardware GPU acceleration via NVIDIA EGL
   - Ideal for Vast.ai containers

**Software rendering (Xvfb, SwiftShader, Lavapipe) is NOT supported** - too slow for streaming.
