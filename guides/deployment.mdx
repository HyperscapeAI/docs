...
The `scripts/deploy-vast.sh` script handles the full deployment with recent improvements (Feb 26 2026):

**Key Steps:**
1. Configure DNS resolution (some Vast containers use internal-only DNS)
2. Pull latest code from main branch (commit 690ede5: now pulls from main, not hackathon)
3. Restore environment variables after git reset (commits eec04b0, dda4396, 4a6aaaf)
4. Install system dependencies (build-essential, ffmpeg, Vulkan drivers, Chrome Dev)
5. Install Playwright and dependencies
6. Build core packages (physx, decimation, impostors, procgen, asset-forge, shared)
7. Setup Solana keypair from `SOLANA_DEPLOYER_PRIVATE_KEY` (commit 8a677dc)
8. Push database schema with drizzle-kit
9. Start port proxies (socat) for external access
10. **Explicitly unset and re-export stream keys** (commits 7ee730d, a71d4ba, 50f8bec)
11. Start duel stack via PM2
12. Wait for health check to pass
13. Run streaming diagnostics (commit cf53ad4)
