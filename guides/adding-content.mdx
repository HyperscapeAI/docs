...
<Info>
  Crafting recipes support consumables with limited uses (e.g., thread with 5 uses). Station can be "none" or "furnace" (for jewelry).
</Info>
...
<Info>
  Fletching recipes support multi-output via `outputQuantity` (e.g., 15 arrow shafts per log). Quantity selection refers to actions, not output items.
</Info>
...
<Info>
  Runecrafting is instant (no tick delay). Multi-rune levels grant +1 rune per essence at each threshold. Example: At level 22, you get 3 air runes per essence.
</Info>
