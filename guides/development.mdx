...
bun run dev:ai        # Game + ElizaOS with concurrently (simplified)
                      # Runs: game (bun run dev) + ElizaOS (PORT=4001 elizaos start)
...
**Reusable UI Primitive** (goes in `packages/client/src/ui/components/`):
```typescript
...
**Game-Specific Component** (goes in `packages/client/src/game/components/`):
```typescript
// Example: Quest tracker
export function QuestTracker({ world }: { world: ClientWorld }) {
  const quests = useQuestStore((state) => state.activeQuests);
  return <div>{quests.map(renderQuest)}</div>;
}
```
