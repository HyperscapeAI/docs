...
Hyperscape uses a **separate assets repository** to keep the main codebase lightweight.

### Asset Repository Structure

- **Main repo** (`HyperscapeAI/hyperscape`): Tracks manifest JSON files only
- **Assets repo** (`HyperscapeAI/assets`): Contains 3D models, textures, audio (~200MB with Git LFS)
...
3. Uses shallow clone with LFS for faster downloads
4. Manifests are copied from assets repo to main repo
...
if (process.env.CI === 'true') {
  ...
  git clone --depth 1 --no-checkout https://github.com/HyperscapeAI/assets.git
...
<Warning>
The entire `packages/server/world/assets/` directory is gitignored. Never commit binary assets to the main repository.
</Warning>
...
2. Manifests are automatically updated
