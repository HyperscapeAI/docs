...
Comprehensive end-to-end tests validate the complete player journey from login to gameplay:

**Test File**: `packages/client/tests/e2e/complete-journey.spec.ts`
...
Removed conflicting TypeScript overrides from root `package.json`. The build system now relies on workspace protocol and Turbo's dependency resolution.
...
**Linux/Windows Desktop Builds (commit f19a7042):**

Fixed unsigned builds for Linux and Windows:
...
Removed invalid matrix reference from job-level condition. Matrix variables are only available during job execution, not at job scheduling time.
