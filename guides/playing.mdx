...
Access the quest journal via the Quests button (📜) in the sidebar.

### Starting Quests

1. Talk to NPCs with quest markers
2. Read quest description and requirements
3. Click "Accept Quest" to begin
4. Starting items granted automatically (if defined)

### Quest Types

| Stage Type | Description |
|------------|-------------|
| **Dialogue** | Talk to specific NPCs |
| **Kill** | Defeat a certain number of enemies |
| **Gather** | Collect items via gathering skills |
| **Interact** | Use objects or stations |

### Quest Journal

- **Color-coded status**: Red (not started), Yellow (in progress), Green (completed)
- **Progress tracking**: Real-time counters (e.g., "Kill 15 goblins (7/15)")
- **Quest points**: Displayed at top of journal
- **Strikethrough**: Completed steps shown with strikethrough

### XP Lamps

Quest rewards often include XP lamps:

1. Right-click lamp in inventory
2. Select "Rub" action
3. Choose which skill receives XP
4. XP granted immediately

<Info>
  XP lamps let you boost any skill of your choice, making them valuable rewards for focused training.
</Info>
