...
# Deployment
NODE_ENV=production
COMMIT_HASH=abc123def456  # Git commit hash for tracking
SKIP_ASSETS=true           # Skip asset download in CI/Railway
...
PUBLIC_PLAYER_COLLISION=false       # Enable player-to-player collision
PUBLIC_MAX_UPLOAD_SIZE=12           # Max upload size in MB
DISABLE_RATE_LIMIT=false            # Disable rate limiting (dev only!)

# Advanced
WORLD=world                         # World folder to load
SYSTEMS_PATH=/path/to/custom/systems  # Custom systems directory
COMMIT_HASH=abc123                  # Git commit (auto-set by CI)
```

### CI/Production Variables

These variables control build and deployment behavior:

| Variable | Purpose | Default |
|----------|---------|---------|
| `CI` | Detected CI environment, skips asset download | `false` |
| `SKIP_ASSETS` | Explicitly skip asset download | `false` |
| `NODE_ENV` | Environment mode (development/production) | `development` |
| `RAILWAY_ENVIRONMENT` | Railway deployment environment | - |
| `TURBO_FORCE` | Force fresh Turbo builds (no cache) | `false` |

**Asset Download Behavior**:
- **Local Development**: Assets downloaded via Git LFS during `bun install`
- **CI/Railway**: Assets skipped, manifests fetched from CDN at runtime
- **Production**: Manifests fetched from `PUBLIC_CDN_URL` at server startup
...
# Optional
PUBLIC_ENABLE_FARCASTER=true        # Enable Farcaster integration
DEBUG_RPG=0                         # Disable debug logging
```

## CORS Configuration

The server accepts requests from multiple origins for development and production:
...
- `http://hyperscape.pages.dev` - Cloudflare Pages (HTTP fallback)
...
**Configuration Location**: `packages/server/src/startup/http-server.ts`

### Custom Domains

Add custom domains via environment variable:

```bash
PUBLIC_APP_URL=https://custom.example.com
```

This automatically adds the domain to the CORS allowlist.
