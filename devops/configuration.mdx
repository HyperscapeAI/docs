...
PUBLIC_CDN_URL=https://your-cdn.com  # Required for manifest fetching
...
<Info>
**Manifest Fetching**: In production (CI=true), the server fetches manifests from `PUBLIC_CDN_URL` at startup. Manifests are downloaded to `packages/server/world/assets/manifests/` and cached for the server lifetime.
</Info>
