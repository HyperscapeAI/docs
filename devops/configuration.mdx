...
# CORS (automatically configured for Cloudflare Pages)
# The server includes these domains in the CORS allowlist:
# - https://hyperscape.pages.dev (production)
# - https://*.hyperscape.pages.dev (preview deployments)
```

### CORS Configuration

For production deployments with separate frontend and backend domains, configure CORS in the server:

```typescript
// packages/server/src/startup/http-server.ts
const allowedOrigins = [
  'http://localhost:3333',           // Local development
  'https://hyperscape.pages.dev',    // Cloudflare Pages production
...
<Info>
Cloudflare Pages preview deployments use subdomains like `abc123.hyperscape.pages.dev`. The wildcard pattern `*.hyperscape.pages.dev` allows all preview deployments to access the API.
</Info>
...
The repository includes automated workflows for code quality, documentation, and deployment:
...
**Features:**
- Triggers only when server-related files change
- Uses Railway GraphQL API for deployment
- Monitors deployment status and reports failures
