...
# Admin Access (REQUIRED for production security - March 2026)
ADMIN_CODE=your-admin-code         # Prevents unauthorized admin access
                                   # Required for /admin/maintenance/*, /admin/logs, /admin/restart
                                   # Without this, admin endpoints return 403 Forbidden
...
# Oracle Configuration (March 2026)
ORACLE_SETTLEMENT_DELAY_MS=7000   # Delay oracle publish to sync with stream (default: 7000ms)
                                  # Prevents oracle publishing before stream viewers see outcome
                                  # Typical stream latency: 7-10 seconds
                                  # Added in PR #1015 (commit 38c8c89)

# Logger Configuration (March 2026)
LOGGER_MAX_ENTRIES=1000           # Ring buffer size for live log streaming (default: 1000)
                                  # Accessible via GET /admin/logs endpoint
                                  # Stores most recent log entries in memory
                                  # Added in PR #1015 for admin dashboard live logs
```
...
# Chrome Capture Configuration (Updated March 2026)
STREAM_CAPTURE_MODE=mediarecorder            # mediarecorder (default), cdp, or webcodecs
                                             # Changed from cdp to mediarecorder (commit 72c667a)
                                             # mediarecorder: canvas.captureStream() → WebSocket → FFmpeg
                                             # cdp: Chrome DevTools Protocol screencast (may stall under Xvfb)
                                             # Requires internalCapture=1 URL param for mediarecorder mode
STREAM_CAPTURE_HEADLESS=false                # false (Xorg/Xvfb), new (headless modes)
STREAM_CAPTURE_USE_EGL=false                 # true for headless EGL mode
STREAM_CAPTURE_CHANNEL=chrome-beta           # Chrome Beta (stable) - changed from chrome-unstable (commit 547714e)
STREAM_CAPTURE_ANGLE=default                 # default (auto-select) - changed from vulkan (commit 547714e)
                                             # default: auto-selects best backend (Vulkan, OpenGL, D3D11)
                                             # vulkan: native Vulkan (may crash on some systems)
                                             # gl: OpenGL backend
                                             # swiftshader: CPU fallback
STREAM_CAPTURE_EXECUTABLE=/usr/bin/google-chrome-beta  # Explicit Chrome path (optional)
...
# ElizaCloud (unified model access - recommended)
...
# Unified API for 13 frontier models (GPT-5, Claude 4.6, Gemini 3.1, Grok 4, etc.)
# Replaces individual provider keys for duel arena agents
ELIZAOS_CLOUD_API_KEY=your-elizacloud-api-key  # Single key for all models
...
# Streaming Viewer Access Control (March 2026)
# Optional secret token for trusted live WebSocket viewers/capture clients
# when delayed public mode is enabled. Loopback viewers are always allowed.
STREAMING_VIEWER_ACCESS_TOKEN=replace-with-random-secret-token

# Stream Destinations Auto-Detection (March 2026)
# Automatically enables destinations based on configured stream keys
# Uses || logic: TWITCH_RTMP_STREAM_KEY || TWITCH_STREAM_KEY
# Auto-detection added in commit 41dc606
STREAM_ENABLED_DESTINATIONS=twitch,kick,x       # Comma-separated list (auto-detected if not set)

# MediaRecorder Streaming Mode (March 2026)
...
# Without internalCapture=1, browser logs 'Bridge capture disabled' and sends no frames

# PostgreSQL Connection Pool (commit 24fa8a5, Mar 10 2026)
# Increased pool size to prevent database timeouts under high load
POSTGRES_POOL_MAX=20                            # Max connections (up from 10)
POSTGRES_POOL_MIN=2                             # Min idle connections
                                                # Prevents "timeout exceeded when trying to connect" errors
                                                # Required for concurrent agent queries during duel cycles
...
```javascript
// From ecosystem.config.cjs (updated Mar 9-10 2026)
// Loads secrets from /tmp/hyperscape-secrets.env at config load time
...
module.exports = {
  apps: [{
    ...
    env: {
      ...
      DUEL_DATABASE_MODE: process.env.DUEL_DATABASE_MODE || "remote",
      DATABASE_URL: process.env.DATABASE_URL || "",
      USE_LOCAL_POSTGRES: process.env.USE_LOCAL_POSTGRES || "false",
      
      // Display for Xvfb (commit 704b955)
      DISPLAY: process.env.DISPLAY || ":99",
      
      PUBLIC_CDN_URL: "https://assets.hyperscape.club",
      
      // PostgreSQL Connection Pool (commit 24fa8a5)
      POSTGRES_POOL_MAX: "20",  // Up from 10 to prevent timeouts under high load
      POSTGRES_POOL_MIN: "2",   // Minimum idle connections
      
      // Streaming configuration (commits 72c667a, 547714e, 41dc606)
      ...
      // Stream keys (explicitly forwarded from environment - commit 41dc606)
...
### Secrets Loading and Stream Key Management (Updated March 2026)

**PM2 Secrets Loading** (Commit 684b203):

PM2 doesn't reliably inherit exported environment variables from the deploy shell script. The `ecosystem.config.cjs` now reads secrets directly from `/tmp/hyperscape-secrets.env` at config load time:
...
After loading secrets, the config automatically detects whether `DATABASE_URL` points to a remote host:

```javascript
// Auto-detect DUEL_DATABASE_MODE from DATABASE_URL
...
**Stream Destination Auto-Detection** (Commit 41dc606):

Stream destinations are auto-detected from available stream keys using `||` logic:
...
- Automatic streaming destination detection from configured keys
- No need to manually set `STREAM_ENABLED_DESTINATIONS`
