...
The server automatically fetches game data manifests from the CDN at startup.
...
- Root: `npcs.json`, `stores.json`, `world-areas.json`, `prayers.json`, etc.
...
- Content comparison: Only changed files are written
- HTTP cache headers: 5-minute cache with revalidation
- Startup logs show: `X fetched, Y updated, Z failed`

<Info>
  This allows updating game content by uploading new manifests to R2 without redeploying the server.
</Info>

## CORS Configuration

The server automatically configures CORS for all deployment environments.
...
**Custom domains:**
- Any domain set in `PUBLIC_APP_URL` environment variable

### Configuration Location

CORS is configured in `packages/server/src/startup/http-server.ts`:
...
<Warning>
  If you deploy to a custom domain, add it to `PUBLIC_APP_URL` in your server environment variables.
</Warning>
