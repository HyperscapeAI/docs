...
SKIP_ASSETS=true  # Skip asset download in CI/production (assets served from CDN)
...
# Deployment
COMMIT_HASH=git-commit-sha  # For deployment tracking
```

### CI/Production Environment Variables

When deploying to Railway, Vercel, or other platforms, set these variables to skip local asset downloads:

```bash
CI=true                    # Detected by ensure-assets.mjs
SKIP_ASSETS=true          # Explicit skip flag
```

The `ensure-assets.mjs` script automatically detects CI environments:
...
<Info>
In production, assets are served from CDN instead of being bundled with the server. Set `PUBLIC_CDN_URL` to your asset hosting URL.
</Info>
