...
**Solution**: Update to latest code (commit f355276 fixed shared/procgen cycle). The build system handles this automatically with resilient patterns:

- **procgen ↔ shared**: Moved procgen from dependencies to optional peerDependencies in shared
  - Breaks turbo graph cycle
  - Import still resolves at runtime (both packages always installed together)
- **plugin-hyperscape ↔ shared**: Uses `--skipLibCheck` in declaration generation
- Packages produce partial output sufficient for downstream consumers
...
**Recent Fix:**
- Commit f355276 (Feb 25, 2026): Broke shared → procgen → shared cycle
...
**Technical Details:**
- Increased CDP stall threshold from 2 → 4 intervals (120s)
- Added soft CDP recovery: restart screencast without browser/FFmpeg teardown
- Increased FFmpeg MAX_RESTART_ATTEMPTS from 5 → 8
- Increased CAPTURE_RECOVERY_MAX_FAILURES default from 2 → 4
...
**Technical Details:**
- Renderer now tries `maxTextureArrayLayers: 2048` first
