...
**Symptom**: Server migration code fails with "table already exists" errors in CI

**Cause**: Running `drizzle-kit push` separately creates tables without populating migration journal, causing server's migration code to fail on re-creation attempts

**Solution**: Remove `drizzle-kit push` from CI workflow (commit b5d24942 fixed this):

```yaml
# ❌ Don't do this in CI
- run: bunx drizzle-kit push

# ✅ Let server handle migrations during startup
- run: bun run start
```

**Technical Details:**
- Server handles its own migrations during startup via `migrate()` function
- Running `drizzle-kit push` separately bypasses migration journal
- Server's migration code then tries to re-create tables, causing conflicts
- CI should only run tests, not push schema changes
