...
**Cause**: Bugs in IndexedDB processed model cache (fixed in PR #935)
...
**Technical Details:**
- **Missing objects**: Models with duplicate mesh names (e.g., "Cube") had only the last mesh appear. Fixed by using `Map<Object3D, number>` identity map instead of `findIndex`-by-name
- **Lost textures**: Textures serialized as ephemeral `blob:` URLs became invalid after page refresh. Fixed by extracting raw RGBA pixels via canvas `getImageData` and restoring as `DataTexture`
- **Grey trees**: `instanceof MeshStandardMaterial` check failed for `MeshStandardNodeMaterial` in WebGPU builds. Fixed with duck-type property check
- Cache version bumped to 3 to invalidate broken entries
- File: `packages/shared/src/utils/rendering/ModelCache.ts`
- Commit: c98f1cc (Feb 25, 2026)
