...
3. Missing Xorg or EGL support
4. WebGPU not available
5. Incorrect headless mode flag passing

**Solutions**:

The deployment system now automatically detects and configures GPU rendering (commits dd649da, e51a332, 30bdaf0, 725e934, 012450c, Feb 27-28 2026):
...
# Manual headless EGL mode (for containers without DRM/DRI or when Xorg falls back to swrast)
export DISPLAY=
export STREAM_CAPTURE_HEADLESS=new
export STREAM_CAPTURE_USE_EGL=true
export GPU_RENDERING_MODE=headless-egl
export VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/nvidia_icd.json
# Uses Chrome's --headless=new with --use-gl=egl --ozone-platform=headless
```
...
# Check Vulkan support (with NVIDIA-only ICD)
export VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/nvidia_icd.json
vulkaninfo --summary
...
# Check Xorg is running (Xorg mode)
xdpyinfo -display :99

# Test Chrome with GPU
...
**Recent Fixes:**
- **Commit 012450c** (Feb 28, 2026): Use headless=new with EGL for WebGPU without Xorg
- **Commit 725e934** (Feb 27, 2026): Detect Xorg swrast fallback and switch to headless EGL
- **Commit dd649da** (Feb 27, 2026): Fix headless mode flag passing (args vs Playwright option)
- **Commit e51a332** (Feb 27, 2026): Added headless EGL mode for containers without DRM
- **Commit 30bdaf0** (Feb 27, 2026): Require NVIDIA Xorg, no software rendering fallback
- **Commit 5fe4a18** (Feb 27, 2026): Add robust Xorg/Xvfb fallback handling
- **Commit 263bfc5** (Feb 27, 2026): Use Xorg instead of Xvfb for NVIDIA GPU rendering
- **Commit 89b78e3** (Feb 27, 2026): Use NVIDIA hardware Vulkan instead of SwiftShader

**Headless Mode Flag Fix (commit dd649da):**

Playwright's `headless` option only accepts boolean, not string. For Chrome's new headless mode (`--headless=new`), we must:
1. Pass `headless: false` to Playwright
2. Add `--headless=new` to the args array manually

```typescript
// ❌ Old: Doesn't work (Playwright rejects string)
const browser = await chromium.launch({
  headless: 'new'  // TypeError: headless must be boolean
});

// ✅ New: Pass as arg, not option
const browser = await chromium.launch({
  headless: false,  // Tell Playwright not to add --headless
  args: ['--headless=new']  // Add Chrome's new headless mode manually
});
```
...
**Symptom**: Stream has video but no audio, or audio is silent

**Cause**: PulseAudio not configured or Chrome not outputting to virtual sink

**Solution**: Verify PulseAudio setup (commits 3b6f1ee, aab66b0, 7a5fcbc, b9d2e41):
...
# If missing, create virtual sink
...
echo $STREAM_AUDIO_ENABLED
# Should be: true

echo $PULSE_AUDIO_DEVICE
# Should be: chrome_audio.monitor
```
...
**Graceful Fallback (commit 7a5fcbc):**

The system gracefully falls back to silent audio if PulseAudio is unavailable:

```bash
# Check if PulseAudio is available before using
...
**See Also:** [Audio Streaming Guide](/devops/audio-streaming) for complete PulseAudio configuration

---

### GPU Rendering Black Screen

**Symptom**: Stream shows black screen instead of game, or WebGPU fails to initialize

**Cause**: GPU not accessible, wrong rendering mode, or software rendering fallback

**Solution**: Verify GPU rendering configuration (commits 725e934, 7140966, dd649da, e51a332, 30bdaf0, Feb 27 2026):

```bash
# Check GPU rendering mode
echo $GPU_RENDERING_MODE
# Should be: xorg or headless-egl (NOT xvfb or swrast)
...
**Headless EGL Mode Verification:**
```bash
# Verify environment
echo $DISPLAY
# Should be empty

echo $STREAM_CAPTURE_HEADLESS
# Should be: new

echo $STREAM_CAPTURE_USE_EGL
# Should be: true

# Test Chrome with EGL
google-chrome-unstable --headless=new --use-gl=egl \
  --ozone-platform=headless --enable-unsafe-webgpu \
  --screenshot=test.png https://get.webgl.org/webgpu/
```

**Common Issues:**

1. **Xorg using software rendering (swrast)**:
   - Deployment script auto-detects and switches to headless EGL
   - Check logs for: "Xorg using SOFTWARE RENDERING - switching to headless EGL"
   - Verify GPU_RENDERING_MODE changed to headless-egl

2. **No DRI devices in container**:
   - Headless EGL mode is the solution
   - Works without /dev/dri access
   - Requires NVIDIA EGL library

3. **Wrong headless mode flag**:
   - Playwright's `headless` option only accepts boolean
   - For `--headless=new`, pass `headless: false` to Playwright and add `--headless=new` to args
   - Fixed in commit dd649da
...
**Symptom**: Xorg starts successfully but stream is black or very slow

**Cause**: NVIDIA driver failed to initialize, Xorg fell back to modesetting with swrast (software rendering)

**Solution**: Update to latest code (commit 725e934 fixed this). The deployment script now detects swrast fallback:

```bash
# Check Xorg log for swrast initialization
...
- Software rendering is too slow for WebGPU streaming (<5 FPS)
- Automatic detection prevents silent performance degradation
- Headless EGL provides hardware acceleration without X server
- Common in containers without full DRM access

**Diagnostic Output:**
```bash
[deploy] ✗ Xorg started but using SOFTWARE RENDERING (swrast) - not usable for WebGPU
[deploy] NVIDIA driver failed to initialize, falling back to headless EGL...
[deploy] Xorg errors:
...
### Headless EGL Environment Variables Not Preserved (Commit 7140966)

**Symptom**: PM2 starts with wrong DISPLAY or missing EGL flags after headless EGL detection

**Cause**: PM2 setup overrode DISPLAY to :99 even when headless EGL mode was detected

**Solution**: Update to latest code (commit 7140966 fixed this). The deployment script now preserves headless EGL env vars:

```bash
# Don't override DISPLAY if it was set to empty for headless EGL
if [ -z "$DISPLAY" ]; then
    # Headless EGL mode - keep DISPLAY empty
    export STREAM_CAPTURE_HEADLESS=new
    export STREAM_CAPTURE_USE_EGL=true
else
    # Xorg mode - DISPLAY already set to :99
    export STREAM_CAPTURE_HEADLESS=false
    export STREAM_CAPTURE_USE_EGL=false
fi

# Preserve in PM2 config
bunx pm2 start ecosystem.config.cjs
```

**Why This Matters:**
- DISPLAY must be empty for headless EGL mode
- Setting DISPLAY=:99 breaks EGL rendering
- STREAM_CAPTURE_HEADLESS and STREAM_CAPTURE_USE_EGL must be preserved
- PM2 config now reads from environment instead of hardcoding

### Audio Dropouts or Desync

**Symptom**: Intermittent audio dropouts or audio gradually drifts out of sync with video

**Cause**: Buffer underruns or timing issues
...
**See Also:** [Audio Streaming Guide](/devops/audio-streaming) for troubleshooting
...
### Gathering Tools Not Showing in Hand

**Symptom**: Woodcutting and mining don't display tools in hand during gathering, only fishing shows tools

**Cause**: Fishing-only gate in `GATHERING_TOOL_SHOW/HIDE` event handlers

**Solution**: Update to latest code (commit 1f789cb fixed this)

**Technical Details:**
- Removed fishing-only gate from `ClientGraphics.ts`
- All gathering skills (woodcutting, mining, fishing) now display correct tool
- Tool overrides equipped weapon during gathering action
- Visual feedback matches actual tool being used

**Impact**: Woodcutting shows hatchet, mining shows pickaxe, consistent with fishing rod display

**Files Changed**: 1 file, 2 additions, 4 deletions

### Pickaxe Can Cut Trees / Hatchet Can Mine Rocks

**Symptom**: Cross-skill tool usage allowed (pickaxe for woodcutting, hatchet for mining)

**Cause**: Substring matching in `itemMatchesToolCategory()` - "pickaxe" contains "axe"

**Solution**: Update to latest code (PR #1098 fixed this)

**Technical Details:**
- **Manifest-Based Validation**: Each tool in `tools.json` declares its skill explicitly
- **Primary Path**: `getExternalTool()` lookup with skill comparison
- **Fallback Guards**: Hatchet rejects "pickaxe"/"pick", pickaxe rejects "hatchet"
- **Fishing Exact Match**: Fishing tools require exact ID match (not interchangeable)
- **Warn-Once Logging**: Bounded Set (max 50 entries) prevents log flooding

**Implementation** (`packages/shared/src/systems/shared/entities/gathering/ToolUtils.ts`):
```typescript
// Manifest lookup (primary path)
...
**Impact**: 
- Prevents cross-skill tool usage (OSRS-accurate)
...
### Mob Names Show Duplicate Levels

**Symptom**: Context menus show "Attack Bandit (Lv8) (Level: 8)" with duplicate level display

**Cause**: Mob names like "Bandit (Lv8)" already contain level, context menu adds "(Level: 8)" again

**Solution**: Update to latest code (PR #1097 fixed this)

**Technical Details:**
- Strip trailing `(Lv#)` suffix from mob display names before building context menu
- Authoritative level from mob data still shown in full "(Level: X)" format
- Regex pattern: `/\s*\(Lv\d+\)\s*$/u`
...
### Resources Not Respawning Reliably

**Symptom**: Trees, rocks, or fishing spots don't respawn after depletion, or respawn timing is inconsistent

**Cause**: Legacy `setTimeout`-based respawn in `ResourceEntity.deplete()` was unreliable

**Solution**: Update to latest code (PR #1099 fixed this)

**Technical Details:**
- **Tick-Based Respawn**: `ResourceSystem.processRespawns()` is now the single source of truth
- **Deterministic Timing**: Respawn based on tick counting, not setTimeout
- **OSRS-Accurate**: Matches Old School RuneScape tick-based mechanics
- **Manifest Depletion**: Mining reads `depleteChance` from manifest (supports rune essence rocks with `depleteChance: 0`)

**Implementation**:
```typescript
// ResourceEntity.deplete() - no setTimeout
public deplete(): void {
  this.isDepleted = true;
  this.depletedAtTick = this.world.currentTick;
  // ResourceSystem.processRespawns() handles respawn
}

// ResourceSystem.processRespawns() - tick-based
public processRespawns(): void {
  const currentTick = this.world.currentTick;
  for (const resource of this.world.entities.values()) {
    if (!(resource instanceof ResourceEntity)) continue;
    if (!resource.isDepleted) continue;
    
    const ticksSinceDepleted = currentTick - resource.depletedAtTick;
...
**Impact**:
- Eliminates race conditions and missed respawns
- Predictable respawn timing for testing
...
- Client becomes sluggish after extended play
- GPU memory warnings in console
- Browser tab crashes after hours of gameplay

**Diagnosis**:

```bash
# Server memory monitoring
node --expose-gc packages/server/src/index.ts
# Watch RSS memory in htop or Activity Monitor
...
### Common Memory Leak Patterns (Fixed in Feb 2026)

<AccordionGroup>
  <Accordion title="Event Listeners Not Removed">
    **Pattern**: `world.on()` or `addEventListener()` without cleanup
    
    **Fix**: Store handler reference and remove on destroy
    ...
    class MySystem {
      ...
      init() {
        this.removeListener = world.on('event', this.handler);
      }
    ...
    **Files Fixed**: EventBridge, GameTickProcessor, TradingSystem, AgentManager, AutonomousBehaviorManager, ColliderComponent, MobEntity, Socket, ClientLiveKit
  </Accordion>

  <Accordion title="Intervals Not Cleared">
    **Pattern**: `setInterval()` or `setTimeout()` without cleanup
    
    **Fix**: Store interval ID and clear on destroy
    ...
    **Files Fixed**: Logger, PlayerTokenManager, HealthBars, ProjectileRenderer, ThreeResourceManager
  </Accordion>

  <Accordion title="GPU Resources Not Disposed">
    **Pattern**: Three.js geometries, textures, materials not disposed
    
    **Fix**: Call `.dispose()` on all GPU resources
    ...
    **Files Fixed**: ModelCache, XPDropSystem, DuelCountdownSplatSystem, HealthBars, ProjectileRenderer
  </Accordion>

  <Accordion title="Unbounded Collections">
    **Pattern**: Maps/Sets growing without bounds
    
    **Fix**: Implement size limits with LRU eviction
    ...
    class MyCache {
      ...
      add(key: string, data: Data) {
        if (this.cache.size >= this.MAX_SIZE) {
          // Evict oldest 25%
          const toRemove = Math.floor(this.MAX_SIZE * 0.25);
    ...
    **Files Fixed**: AggroSystem, StarterChestEntity, ActivityLoggerSystem
  </Accordion>

  <Accordion title="Async State Updates After Unmount">
    **Pattern**: React setState after component unmounted
    
    **Fix**: Use cancelled flag to guard async updates
    ...
    **Files Fixed**: EmbeddedGameClient
  </Accordion>

  <Accordion title="World Init/Destroy Race Condition">
    **Pattern**: Cleanup runs before async init completes
    
    **Fix**: Two-flag handshake (initComplete + needsCleanup)
    ...
    class World {
      ...
      async init() {
        try {
          ...
          this.initComplete = true; // Still set to allow cleanup
          throw err;
        }
      }
    ...
    **Files Fixed**: GameClient world initialization
  </Accordion>
</AccordionGroup>
...
- [ ] Track all event listeners and remove them in `destroy()`
- [ ] Clear all intervals and timeouts in `destroy()`
- [ ] Dispose all Three.js resources (geometries, textures, materials)
- [ ] Implement size limits on Maps/Sets that grow with player count
- [ ] Guard async state updates with cancelled flags in React components
- [ ] Follow SystemBase cleanup pattern
- [ ] Test for leaks during hot reload
- [ ] Monitor memory usage in long-running sessions

**See Also**: [Memory Management Best Practices](/changelog-feb-2026#memory-management-best-practices) in February 2026 changelog
