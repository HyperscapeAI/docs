...
- Build: Vite 8.0.0 (upgraded from 6.4.1), Turbo
...
**Change** (Commit 7519105): Major Docker build improvements for multi-service deployment.
...
2. **Bun Version**: Old Bun 1.1.38 couldn't run Vite builds
3. **Node Binary**: `ensure-assets.mjs` failed because bun-only base image doesn't have node binary
4. **better-sqlite3 Crash**: Native build segfaults under QEMU cross-compilation
5. **Workspace Symlinks**: Docker COPY flattens Bun workspace symlinks in `node_modules`
6. **Bun 1.3 Per-Package node_modules**: Bun 1.3 no longer hoists all deps to root

**Fixes**:
```dockerfile
# Builder stage - Bun 1.1.38 → 1.3.10
...
```bash
# Strip better-sqlite3 from manifests before install (segfaults under QEMU)
bun remove better-sqlite3 --cwd packages/server
```

**Impact**: 
- Docker image now supports both server and client deployments
- Vite builds work correctly with Bun 1.3.10
- Workspace packages resolve at runtime
- No QEMU cross-compilation crashes
- Proper dependency isolation per package
...
STREAM_CAPTURE_CHANNEL=chrome-beta  # Chrome Beta for WebGPU stability on Linux
...
**Impact**:
- Vite 8.0 brings significant performance improvements and new features
- Better VRM avatar support with latest three-vrm
