...
<Steps>
  ...
  <Step title="Start the CDN (serves game assets)">
    ```bash
    bun run cdn:up
    ```
    
    This starts a Docker nginx container serving assets on port 8080.
  </Step>
  ...
  <Step title="Start the game">
    ...
    **Development mode (with hot reload):**
    ...
    - CDN: `http://localhost:8080` (Docker)
  </Step>
</Steps>
...
Access the agent dashboard at `http://localhost:3333` and click the Dashboard icon to:
- Monitor agent status, goals, and progress
- View real-time skills, position, and activity
- Control agents with Stop/Resume buttons
- Send commands via Quick Action menu
- Watch agents play in the viewport
...
|---------|------|-------------|------------|
| Client | 3333 | Vite dev server with hot reload | ✅ Yes |
| Server | 5555 | Game server (Fastify + WebSockets) | ✅ Yes |
| PostgreSQL | 5432 | Database (Docker) | ✅ Yes |
| CDN | 8080 | Asset server (Docker nginx) | ❌ Manual |

<Info>
The CDN must be started manually with `bun run cdn:up` before running the game. It serves 3D models, audio, textures, and manifests required by the client and server.
</Info>
...
```mermaid
flowchart TD
    A[CDN :8080] -->|Manifests| B[Server :5555]
    A -->|Assets| C[Client :3333]
    B -->|WebSocket| C
    D[PostgreSQL :5432] -->|Data| B
```

The server fetches manifests from the CDN at startup, so the CDN must be running first.
...
6. **Train gathering skills** - Chop trees, mine ore, fish, or explore to train Agility
7. **Train artisan skills** - Light fires, cook food, smelt bars, smith equipment
8. **Explore the world** - Gain Agility XP by moving (1 XP per 2 tiles)
9. **View skill guides** - Click any skill in the Skills Panel to see unlocks
10. **Press F5** - Toggle FPS debug panel to monitor performance
