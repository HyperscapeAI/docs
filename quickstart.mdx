---
title: "Quickstart"
description: "Get Hyperia running locally in minutes"
icon: "zap"
---
...
<Warning>
  Hyperia requires WebGPU for rendering. All shaders use Three.js Shading Language (TSL) which only compiles to WebGPU. Browsers without WebGPU support will show an error screen with upgrade instructions. **Note:** Safari 17 support was removed - Safari 18+ (macOS 15+) is now required.
</Warning>
...
<Steps>
  <Step title="Clone the repository">
    ```bash
    git clone https://github.com/PlayHyperia/hyperforge.git
    cd hyperforge
    bun install
    ```
  </Step>
...
<Info>
  The betting stack has been moved to a separate repository: [HyperscapeAI/hyperbet](https://github.com/HyperscapeAI/hyperbet). The duel oracle remains in Hyperia for verifiable outcome publishing.
</Info>
