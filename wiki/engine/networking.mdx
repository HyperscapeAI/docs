...
- `inCombat` → `c` saves 7 bytes per packet
- `combatTarget` → `ct` saves 10 bytes per packet
...
See [Combat System - Critical Bug Fixes](/wiki/game-systems/combat#critical-bug-fixes) for complete details.
