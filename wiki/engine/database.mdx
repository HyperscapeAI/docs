...
The `player_deaths` table tracks active player deaths for crash recovery:

```typescript
export const playerDeaths = pgTable('player_deaths', {\n  playerId: text('player_id').primaryKey(),
  gravestoneId: text('gravestone_id'),
  groundItemIds: text('ground_item_ids').array(),  // Array of entity IDs
  positionX: real('position_x').notNull(),
  positionY: real('position_y').notNull(),
  positionZ: real('position_z').notNull(),
  timestamp: bigint('timestamp', { mode: 'number' }).notNull(),
  zoneType: text('zone_type').notNull(),           // safe, dangerous, pvp
  itemCount: integer('item_count').notNull(),
  
  // Crash recovery fields (added in migration 0017)
  items: jsonb('items').$type<Array<{ itemId: string; quantity: number }>>(),
  killedBy: text('killed_by'),                     // Killer name for gravestone
  recovered: boolean('recovered').default(false),  // Has been processed
...
**Purpose:**
- Prevents item duplication if server crashes during death handling
- Stores item data for gravestone recreation after crash
- Tracks recovery status to prevent duplicate processing
- Displays killer name on gravestone

**Lifecycle:**
1. Created when player dies (atomic acquisition)
2. Updated when items are looted (`onItemLooted`)
3. Updated when gravestone expires (`onGravestoneExpired`)
4. Deleted when all items recovered (`clearDeathLock`)
5. Recovered on server restart if `recovered = false`

<Warning>
Death locks persist until items are fully looted. This is critical for preventing item duplication on server crashes.
</Warning>
