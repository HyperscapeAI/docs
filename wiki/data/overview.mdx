...
├── prayer-alter/prayer-alter.glb → Prayer altar
```
...
| **Cooking Range** | Cook food with reduced burn chance | `asset://models/cooking-range/cooking-range.glb` | 1.0 | 0.5 |
| **Bank Chest** | Store items | `asset://models/bank-chest/bank-chest.glb` | 0.5 | 0.75 |
...
│   └── metadata.json       # Model metadata
├── cooking-range/
│   └── cooking-range.glb   # Optimized model (scale: 1.0, yOffset: 0.5)
├── bank-chest/
│   └── bank-chest.glb      # Optimized model (scale: 0.5, yOffset: 0.75)
└── prayer-alter/
