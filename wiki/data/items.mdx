...
Items are defined in **JSON manifests** and loaded at runtime. This includes weapons, armor, tools, consumables, and resources.

<Info>
Item data is managed in `packages/shared/src/data/items.ts` and loaded from `world/assets/manifests/items.json`.
</Info>

## Data Loading

Items are **NOT hardcoded**. The `ITEMS` map is populated at runtime from JSON manifests:

```typescript
// From items.ts
export const ITEMS: Map<string, Item> = new Map();

// Populated by DataManager from JSON manifests
await dataManager.initialize();
// Loads from items/ directory (weapons.json, tools.json, resources.json, food.json, misc.json)
// Falls back to items.json if directory doesn't exist
```
...
<Steps>
  <Step title="Choose Category">
    Determine which category file your item belongs in:
  ...
  <Step title="Add to JSON Manifest">
    Add entry to the appropriate file in `packages/server/world/assets/manifests/items/`
  </Step>
  <Step title="Add Recipe (if applicable)">
    If the item is craftable, add recipe to appropriate file in `recipes/`:
...
To add a new smithable item:

1. **Add item to `items/weapons.json`**:
```json
{
...
2. **Add recipe to `recipes/smithing.json`**:
```json
{
...
3. **Restart server** - The item and recipe will be available in-game.
