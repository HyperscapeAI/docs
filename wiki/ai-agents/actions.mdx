...
**Special Features:**
- Supports "drop all" commands (e.g., "drop all logs", "drop everything")
- Word boundary matching prevents false matches (e.g., "bronze pickaxe" won't match "bronze hatchet")
- Limited to 50 items per drop-all operation to prevent memory leaks
- Consecutive failure tracking stops operation after 3 failures
...
**Usage**: Loot drops, collect resources.

**Features:**
- Automatically walks to items beyond pickup range (4m)
- Scoring system matches best item when multiple items nearby
