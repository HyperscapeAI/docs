...
This page documents all **22 actions** available to ElizaOS agents in Hyperscape. Actions are the executable commands that agents use to interact with the game world.

<Note>
**ElizaOS 1.7.0 Compatibility**: All actions are compatible with ElizaOS 1.7.0+. The plugin uses backwards-compatible shims for cross-version support.
</Note>
...
**Cardinal Positioning**: The server requires players to be on a cardinal adjacent tile (North/South/East/West, not diagonal) to chop trees. The action automatically:
1. Finds the 4 cardinal adjacent tiles to the tree
2. Selects the nearest one to the player's current position
3. Moves to that tile before starting to chop

This ensures OSRS-accurate positioning mechanics.
...
**Features:**
- **Single Item**: Drop specific items by name
- **Drop All**: "drop all logs" or "drop everything"
- **Smart Matching**: Word boundary regex prevents false matches
- **Safety Limits**: Maximum 50 items per drop-all operation
- **Error Recovery**: Stops after 3 consecutive failures

**Examples:**
```
"Drop the bronze hatchet"
"Drop all logs"
"Drop everything"
```

<Warning>
Drop-all operations are limited to 50 items to prevent memory leaks and DoS attacks.
</Warning>
...
export const pickupItemAction: Action = {
  ...
  parameters: {
    itemId: { type: "string", optional: true, description: "Specific item to pick up" },
  },
};
```

**Features:**
- **Smart Matching**: Scoring system matches items by word count
  - "bronze pickaxe" scores 20 points (2 words match)
  - "bronze hatchet" scores 10 points (1 word matches)
  - Picks highest scoring item
- **Auto-Walk**: Walks to items beyond pickup range (4m)
- **Distance Stepping**: Handles server's 200-tile movement limit

**Examples:**
```
"Pick up the bronze pickaxe"
"Grab that sword"
"Loot the coins"
```

<Info>
The scoring system uses word boundary regex to prevent false matches. "pick up axe" won't match "Bronze Hatchet" (contains "axe" substring) - it requires exact word matches.
</Info>

**Drop All Support**: The action supports "drop all" commands:
- `"drop all logs"` - Drops all logs in inventory
- `"drop everything"` - Drops all items (limited to 50 items max)
- `"drop all items"` - Same as "drop everything"

**Safety Limits**:
- Maximum 50 items per drop-all operation (prevents DoS)
- Stops after 3 consecutive failures
- 150ms delay between drops

**Item Matching**: Uses word boundary regex to prevent false matches:
- `"drop bronze pickaxe"` → Correctly matches "Bronze Pickaxe" (not "Bronze Hatchet")
- Scoring system: more matching words = higher score
- Falls back to nearest item if no match found
...
export const pickupItemAction: Action = {
  ...
  description: "Pick up an item from the ground nearby",
  parameters: {
    itemId: { type: "string", optional: true, description: "Specific item ID" },
  },
};
```
...
**Auto-Approach**: If item is more than 4 meters away, agent automatically walks to it first.

**Item Matching**: Uses intelligent scoring system:
- Perfect name match: 100 points
- Each matching word: 10 points
- Exact word match bonus: 5 points
- Word boundary checks prevent false matches

**Example**: `"pick up bronze pickaxe"` correctly identifies "Bronze Pickaxe" over "Bronze Hatchet" because both words match (20 points vs 10 points).
...
**Features:**
- **Autonomous Goal Selection**: Agent chooses goals based on context
- **Manual Override**: Dashboard can set specific goals
- **Pause State Blocking**: SET_GOAL is blocked when goals are paused
  - User clicks "Stop" button → goals paused
  - Agent forced to IDLE instead of setting new goals
  - Prevents unwanted autonomous behavior
- **Locked Goals**: User commands create locked goals that persist
  - 60-second grace period for multi-step actions
  - Agent continues executing until action completes or times out
