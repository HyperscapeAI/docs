...
The plugin provides **26 actions** across 9 categories (updated Feb 26 2026):
...
| `bankDepositAllAction` | `actions/banking.ts` | Bulk deposit (keeps tools) |

### Questing Actions <Badge text="New" variant="success" />

| Action | File | Description |
|--------|------|-------------|
| `acceptQuestAction` | `actions/quests.ts` | Accept quest from NPC |
| `completeQuestAction` | `actions/quests.ts` | Complete quest for rewards |
...
Model agent stability fixes:
- Database isolation (force PGLite, prevent schema conflicts)
- 45s initialization timeout prevents indefinite hangs
- Event listener duplication guard prevents memory leaks
- 10s timeout on runtime.stop() with dangling promise cleanup
- Circuit breaker (3 consecutive failures, 8 max reconnect retries)
- Explicit DB adapter close for WASM heap cleanup
