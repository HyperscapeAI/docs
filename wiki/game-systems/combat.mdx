...
The DuelSystem integrates with CombatSystem for rule enforcement:

```typescript
// From packages/server/src/systems/DuelSystem/index.ts
// Other systems check if player is in active duel
const isInDuel = duelSystem.isPlayerInActiveDuel(playerId);

// Check specific rules
if (!duelSystem.canEatFood(playerId)) {
  return { success: false, error: "Food is disabled in this duel" };
}

if (!duelSystem.canMove(playerId)) {
  return { success: false, error: "Movement is disabled" };
}
```
...
- [Duel Arena System](/wiki/game-systems/duel-arena) (complete PvP dueling guide)
- [Inventory System](/wiki/game-systems/inventory) (for food consumption and staking)
