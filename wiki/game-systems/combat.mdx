...
Mobs can use any of the three attack types: melee, ranged, or magic. This is configured in the NPC manifest:
...
**Dual Routing Paths:**
1. **Initial attack**: `MobEntity.performAttackAction()` → `COMBAT_MOB_NPC_ATTACK` event → `handleMobAttack()` → handler
2. **Auto-attack ticks**: `CombatTickProcessor` reads `combatState.weaponType` → `handleAttack()` → handler

Both paths converge on the same handlers, ensuring consistent behavior.
...
export function prepareMobAttack(
  ...
  data: { attackerId: string; targetId: string; attackerType: "mob"; targetType: "player" | "mob" },
  combatRange: number,
...
**Validation Steps:**
1. Resolve attacker and target entities
2. Check both are alive
3. Validate range (uses mob's `combatRange` from manifest)
4. Check attack cooldown
5. Claim cooldown slot
6. Face target
7. Play attack animation

Returns `MobAttackContext` with all validated state, or `null` if any check fails.
