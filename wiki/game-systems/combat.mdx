...
export const MAGIC_STYLE_BONUSES = {
  accurate: { attackBonus: 3, speedModifier: 0, rangeModifier: 0, xpSplit: "magic" },
  longrange: { attackBonus: 1, speedModifier: 0, rangeModifier: 2, xpSplit: "magic_defence" },
  autocast: { attackBonus: 0, speedModifier: 0, rangeModifier: 0, xpSplit: "magic" },
};
```
...
function calculateAccuracy(
  ...
  // Get prayer bonuses for attacker and defender
  const attackerPrayerBonuses = prayerSystem.getCombinedBonuses(attackerId);
  const defenderPrayerBonuses = prayerSystem.getCombinedBonuses(targetId);
  
  // Apply prayer multipliers
  const prayeredAttack = attackerAttackLevel × (attackerPrayerBonuses.attackMultiplier ?? 1);
  const prayeredDefense = targetDefenseLevel × (defenderPrayerBonuses.defenseMultiplier ?? 1);
...
<Info>
Prayer bonuses apply to both attacker and defender, affecting accuracy and defense rolls.
</Info>
