...
### Weapon Type Propagation (PR #934)
...
**Critical Fixes (PR #934):**
- **Keep-alive re-engagement**: DuelCombatAI now periodically re-engages to prevent agents idling when combat state times out (fixes 2H sword attacks)
- **Rune inventory readiness**: Waits for inventory to load from DB before adding runes (prevents silent rune loss)
- **Combat timeout refresh**: Ranged/magic attacks now refresh combat timeout in both `CombatSystem` and `CombatTickProcessor`
- **PvP zone bypass**: Streaming duel combatants bypass PvP zone checks (allows combat in safe zones)
- **Safe zone aggro block**: AggroSystem now blocks aggro and chase on players in safe zones
