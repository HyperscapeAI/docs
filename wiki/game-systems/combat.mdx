...
export const COMBAT_CONSTANTS = {
  ...
  EAT_ATTACK_DELAY_TICKS: 3,          // 1.8s added to attack cooldown when eating
...
Eating food during combat follows OSRS-accurate mechanics with attack delay integration.
...
### Attack Delay When Eating

OSRS rule: Food only adds to **existing** attack delay. If weapon is ready, eating does NOT add delay.
...
1. **Client**: Player clicks "Eat" on food item
2. **Network**: `useItem` packet sent to server
3. **Server Validation**:
   - Check eat delay cooldown (3 ticks)
   - Validate item exists at claimed slot
   - Validate item ID matches slot contents
   - Check rate limiting (3 requests/sec)
4. **Consumption**: Remove item from inventory
5. **Healing**: Apply heal amount (capped at 99 HP)
6. **Attack Delay**: Add 3 ticks to attack cooldown if in combat
7. **Message**: "You eat the shrimp." (shown even at full health)

### Security Features

- **Server-Authoritative**: All validation happens server-side
- **Rate Limiting**: Separate consume rate limiter (3/sec)
- **Input Validation**: Slot bounds, item ID mismatch detection
- **Exploit Prevention**: Heal amount capped at `MAX_HEAL_AMOUNT` (99)
- **Eat Delay Enforcement**: Server tracks cooldowns, rejects spam
