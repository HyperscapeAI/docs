...
The `DuelCombatAI` system manages automated agent duels with proper attack timing:

```typescript
// From DuelCombatAI.ts
// Attack every weapon-speed cycle instead of waiting for re-engagement interval
// Seeds first tick to attack immediately since startCombat() has no auto-attack
private async attackCycle(): Promise<void> {
  const weaponSpeed = this.getWeaponSpeed();
  
  // Seed first tick to attack immediately
  await this.performAttack();
  
  // Continue attacking every weapon-speed cycle
  while (this.isInCombat()) {
    await this.sleep(weaponSpeed);
    await this.performAttack();
  }
}
```
...
**Key Improvements:**
- **Immediate first attack**: Seeds first tick to attack immediately (no delay waiting for re-engagement)
- **Weapon-speed cycles**: Attacks every weapon-speed interval (e.g., 2.4s for longsword, 3.6s for 2H sword)
- **2H sword fix**: 2H swords now attack at correct 6-tick (3.6s) intervals instead of waiting for re-engagement
