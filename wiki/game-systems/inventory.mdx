...
Food items can be consumed to restore health with OSRS-accurate timing mechanics.
...
The `item-helpers.ts` module provides type detection utilities:
...
usesWear(item);      // Armor (head, body, legs, etc.)
...
Context menus use OSRS-accurate color coding:
...
- Right-click context menu with OSRS-style colors
- Left-click primary action (eat, wield, use, etc.)
- Shift-click to drop
- Stack quantity display
- Coin pouch separate display
- Bank note visual styling (parchment background)
