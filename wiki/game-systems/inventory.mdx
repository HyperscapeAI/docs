...
The inventory system works closely with the equipment system for equip/unequip operations.

### Equipment Loading - Single Source of Truth Pattern

To prevent race conditions where equipment disappears on login, the system uses a **single source of truth pattern**:

```typescript
// From character-selection.ts (ServerNetwork)
// 1. Load equipment from database ONCE
const equipment = await equipmentRepo.getByCharacterId(characterId);

// 2. Pass equipment data in PLAYER_JOINED event payload
...
// 3. EquipmentSystem receives equipment from payload (not DB query)
this.subscribe(EventType.PLAYER_JOINED, async (data) => {
  if (data.equipment && data.equipment.length > 0) {
    await this.loadEquipmentFromPayload(data.playerId, data.equipment);
  }
});
```

**Why This Matters:**
- Eliminates race condition where two systems query DB independently
- Ensures equipment state is consistent across all systems
- Prevents "equipment disappearing on login" bug
- Single database query instead of multiple

### Equipment Sync Data Type

```typescript
// From packages/shared/src/types/events/event-payloads.ts
...
- Event payload data (PLAYER_JOINED)
- Network messages (equipmentUpdated)
- Database serialization

### Equipment Persistence

Equipment is saved to database:
- **Auto-save**: Every 30 seconds
- **On equip/unequip**: Immediately after changes
- **On logout**: Final save before disconnect
- **Async destroy**: Awaits all saves before shutdown
...
### Equipment Clearing (Death System)

The equipment system provides atomic clear operations for the death system:

```typescript
// Clear all equipment and return items (for gravestone spawning)
...
<Warning>
The `clearEquipmentAndReturn()` method is critical for death system security. It atomically reads AND clears equipment in one operation, preventing item duplication if the server crashes mid-death.
</Warning>
...
- [Death System](/wiki/game-systems/death) (for equipment clearing)
