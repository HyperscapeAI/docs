...
## Item Usage (Eating, Drinking, etc.)

The inventory system supports server-authoritative item usage for consumables:

```typescript
// Client sends useItem packet
world.network.send("useItem", { itemId, slot });

// Server flow:
// 1. handleUseItem() validates request (rate limiting, slot bounds, item ID)
// 2. Emits INVENTORY_USE event
// 3. InventorySystem.useItem() validates item exists at slot
// 4. Emits ITEM_USED event
// 5. PlayerSystem.handleItemUsed() processes consumption (eat delay, healing, etc.)
```

### Security Features

- **Rate Limiting**: Separate consume rate limiter (3 requests/sec)
- **Input Validation**: Slot bounds checking, item ID mismatch detection
- **Server Authority**: All validation happens server-side
- **Consume-After-Check**: Item removed only after all validation passes

### Noted Items

Noted items (bank notes) have special handling:

```typescript
// Noted items cannot be used as skilling tools
// From ToolUtils.ts
...
<Warning>
Noted items (ending in `_noted`) cannot be equipped or used as tools. They must be un-noted at a bank first.
</Warning>
...
| `INVENTORY_USE` | `playerId`, `itemId`, `slot` | Player using item (eating, drinking) |
