...
Agility XP is gained passively by moving through the world:

| Movement Type | Tiles/Tick | XP Rate | XP/Minute |
|---------------|------------|---------|-----------|
| Walking | 2 | 1 XP per 2 tiles | ~100 XP |
| Running | 4 | 1 XP per 2 tiles | ~200 XP |

<Info>
**XP Batching**: To prevent visual spam, agility XP is granted in batches of 50 XP every 100 tiles traveled. This creates XP drops approximately every 15 seconds while running or 30 seconds while walking.
</Info>

<Warning>
**Death Penalty**: When you die, accumulated tiles toward your next agility XP grant are lost (maximum ~50 XP worth of progress). Already-earned XP is not affected.
</Warning>
...
**Training Method**: Move through the world (walking or running)
- 1 XP per 2 tiles traveled
- XP granted in batches of 50 XP every 100 tiles
- Both walking and running grant XP at the same rate per tile

**Stamina Regeneration Bonus**:
- Base regeneration: 2/sec while walking, 4/sec while idle
- Agility bonus: +1% regeneration per agility level
- Level 99 agility: Nearly double regeneration speed
...
<Info>
**Weight Display**: Your total inventory weight is shown in the Equipment panel. Weight is calculated server-side and synced to the client for accurate stamina calculations.
</Info>

<Tip>
High agility levels help offset the stamina penalty from carrying heavy loads. A level 50 agility player with 50kg of gear will regenerate stamina faster than a level 1 agility player with no gear.
</Tip>
...
Click any skill in the Skills panel to open its Skill Guide, which shows all unlocks for that skill.

**Features**:
- Level requirements for each unlock
- Visual distinction between unlocked (green ✓) and locked (🔒) content
...
<Info>
The Skill Guide Panel fetches unlock data from the server endpoint `/api/data/skill-unlocks`, ensuring server-authoritative progression information.
</Info>
