...
| **Prayer** | Temporary combat buffs | Burying bones, using altars |
...
| **Agility** | Stamina and movement | Moving through the world (1 XP per 2 tiles) |
...
| Skill | Description | Training Methods |
|-------|-------------|------------------|
| **Agility** | Movement efficiency | Walking/running (passive XP gain) |

<Info>
Agility is trained passively through movement and provides stamina regeneration bonuses.
</Info>
...
Players can click any skill in the Skills Panel to open an OSRS-style Skill Guide showing all unlocks at each level.

**Features:**
- Click-to-view unlock progression for any skill
...
- Shows "X of Y unlocks achieved" progress

**Implementation:**
- Client-side component: `packages/client/src/game/panels/SkillGuidePanel.tsx`
- Server API endpoint: `GET /api/data/skill-unlocks`
- Data source: `packages/server/world/assets/manifests/skill-unlocks.json`

<Info>
The Skill Guide Panel fetches unlock data from the server on first load and caches it for the session. This ensures server-authoritative unlock definitions.
</Info>
...
Agility is trained passively through movement:

**Movement XP Tracking**:
- XP granted every 100 tiles traveled
- 1 XP per tile (100 XP per batch)
- Distance resets on death
- Server-authoritative tracking prevents exploits

**Stamina System Integration**:
- **Weight-Based Drain**: Heavier inventory = faster stamina drain
- **Agility Bonus**: +1% stamina regeneration per Agility level
- **Server-Client Sync**: Weight updates forwarded via event bridge

```typescript
// From AgilitySystem (conceptual - integrated into movement)
const TILES_PER_XP_BATCH = 100;
const XP_PER_TILE = 1;

// Grant XP every 100 tiles
if (tilesTraveled >= TILES_PER_XP_BATCH) {
  const xp = TILES_PER_XP_BATCH * XP_PER_TILE;
  skillsSystem.grantXP(playerId, "agility", xp);
  tilesTraveled = 0;  // Reset counter
}
```

**Stamina Regeneration Formula**:
```typescript
const agilityBonus = 1 + (agilityLevel * 0.01);  // +1% per level
const regenRate = baseRegenRate * agilityBonus;
```

<Info>
Agility XP is batched to reduce network traffic and database writes. The 100-tile threshold ensures XP grants are meaningful while preventing spam.
</Info>
...
- Base regen: 2 stamina/second while walking, 4 stamina/second while idle
- Agility bonus: +1% regen per Agility level
- Example: Level 50 Agility = +50% regen = 3 stamina/second while walking
...
<Info>
Weight is calculated server-side and synced to the client via `PLAYER_WEIGHT_CHANGED` events when inventory changes. Stamina calculations are client-side in `PlayerLocal.ts`.
</Info>
...
- **Agility 99**: Agility cape, +99% stamina regeneration
...
The Skill Guide Panel is an OSRS-style UI that displays skill unlock progression. Players can click any skill in the Skills Panel to view what they unlock at each level.

### Features

- **Unlock Progression**: Shows all unlocks from level 1-99
- **Visual States**: 
...
<SkillGuidePanel
  ...
  skillIcon="🪓"
  playerLevel={playerSkills.woodcutting.level}
...
The client fetches skill unlocks from the server:

```typescript
// GET /api/data/skill-unlocks
// Returns: Record<string, SkillUnlock[]>
...
Skill unlocks are loaded from the `skill-unlocks.json` manifest:

```typescript
// From skill-unlocks.ts
...
<Info>
The manifest uses OSRS-accurate British spelling ("defence"), but the UI normalizes to American spelling ("defense") for consistency.
</Info>
