...
| **Agility** | Stamina and movement | Traversing the world (1 XP per 2 tiles) |
...
| Skill | Description | Training Methods |
|-------|-------------|------------------|
| **Prayer** | Activate protective prayers | Burying bones, using prayer at altars |
...
The Prayer skill allows players to activate temporary combat bonuses and protective effects.

### Prayer Points

- **Maximum Prayer Points**: Equal to Prayer level (level 43 = 43 prayer points)
- **Drain Rate**: Each active prayer drains points over time based on its `drainEffect` value
- **Restoration**: Prayer points restore at altars or by drinking prayer potions

### Prayer Categories

| Category | Effect | Examples |
|----------|--------|----------|
| **Offensive** | Attack and strength bonuses | Burst of Strength (+5% strength), Clarity of Thought (+5% attack) |
| **Defensive** | Defense bonuses | Thick Skin (+5% defense), Rock Skin (+10% defense), Steel Skin (+15% defense) |
| **Utility** | Special effects | Protect from Melee, Rapid Heal (future) |

### Prayer Conflicts

Prayers in the same category conflict with each other. Activating a prayer automatically deactivates conflicting prayers (OSRS-accurate behavior).

```typescript
// Example from prayers.json
{
  "id": "thick_skin",
  "name": "Thick Skin",
  "level": 1,
...
- **Burying bones**: Varies by bone type (4-15 XP)
- **Using bones on altars**: 3.5x XP multiplier (future feature)

<Info>
Prayer is defined in `manifests/prayers.json` and managed by `PrayerSystem.ts`.
</Info>
...
The Agility skill rewards players for traversing the game world and ties into the stamina system.

### Movement XP

Players gain Agility XP by moving through the world:

- **XP Rate**: 1 XP per 2 tiles traveled
- **XP Batching**: XP is granted in batches of 50 XP every 100 tiles to prevent visual spam
- **Walking Speed**: ~100 XP/minute (2 tiles/tick)
- **Running Speed**: ~200 XP/minute (4 tiles/tick)

```typescript
// XP batching constants
...
When a player dies, accumulated tiles toward the next XP grant are reset to 0. Already-earned XP is preserved.
...
- **Bonus**: +1% regen per Agility level
...
<Info>
The Agility system creates a meaningful progression loop: higher Agility allows you to run longer, which helps you train Agility faster.
</Info>
...
Players can click any skill in the Skills Panel to open an OSRS-style Skill Guide showing all unlocks for that skill.
...
**Implementation**:
- Client: `packages/client/src/game/panels/SkillGuidePanel.tsx`
- Server API: `GET /api/data/skill-unlocks`
- Data source: `packages/shared/src/data/skill-unlocks.ts`

<Info>
The Skill Guide Panel fetches unlock data from the server to ensure it's always up-to-date with the latest manifest changes.
</Info>
