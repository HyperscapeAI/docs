...
- Batched every 100 tiles (50 XP grant) to reduce visual spam
...
| Level | Total XP | Time Running |
|-------|----------|--------------|
| 10 | 1,154 | ~6 minutes |
| 50 | 101,333 | ~8.5 hours |
| 99 | 13,034,431 | ~1,000+ hours |

<Info>
XP drops appear approximately every 15 seconds while running, or every 30 seconds while walking. Partial progress is preserved between XP grants.
</Info>

<Warning>
Death resets your accumulated tile progress toward the next XP grant (max 50 XP lost). Earned XP is never lost on death.
</Warning>
...
Agility interacts with the weight and stamina mechanics:

### Stamina Drain (Weight-Based)

Carrying heavy items increases stamina drain while running:

**Formula:** `Drain Rate = Base Rate × (1 + Weight × 0.005)`
...
### Stamina Regeneration (Agility-Based)

Higher agility levels increase stamina regeneration:

**Formula:** `Regen Rate = Base Rate × (1 + Agility Level × 0.01)`
...
<Info>
At level 99 agility, stamina regenerates nearly twice as fast as level 1, making it easier to sustain running even with heavy loads.
</Info>
...
Click any skill in the Skills panel to open its guide, which shows:
...
- Next unlock highlight with levels remaining

<Info>
The Skill Guide Panel provides an OSRS-style interface for viewing skill progression and planning your training goals.
</Info>

### Skill Guide Panel

Players can click any skill in the Skills Panel to open an OSRS-style Skill Guide that displays all unlocks for that skill:
...
- Displays progress: "X more levels to unlock: [description]"
...
- Data source: `/api/data/skill-unlocks` endpoint (server-authoritative)
- Unlock data: `packages/server/world/assets/manifests/skill-unlocks.json`

**User Experience:**
```
Skills Panel → Click skill icon → Skill Guide Panel opens
  ├─ Header: [Icon] Skill Name Guide [X]
  ├─ Current Level: Your Level: 45
  ├─ Progress: 12/29 unlocked
  ├─ Next Unlock: 5 more levels to unlock: Rune weapons
  └─ Scrollable List:
      ✓ Lvl 1   Bronze weapons    [item]
      ✓ Lvl 5   Steel weapons     [item]
      ✓ Lvl 40  Rune weapons      [item]
      ➤ Lvl 50  Dragon weapons    [item]  [NEXT]
      🔒 Lvl 60 Godswords         [item]
```
...
<Info>
The Skill Guide Panel fetches unlock data from the server on first load and caches it for the session. This ensures unlock data is always server-authoritative and can be updated without client rebuilds.
</Info>
...
The skill unlocks system handles British/American spelling differences automatically:

```typescript
// From packages/shared/src/data/skill-unlocks.ts
...
**Why this matters:**
- `skill-unlocks.json` uses British spelling "defence" (OSRS-accurate)
- Client UI uses American spelling "defense" (consistent with codebase)
- Normalization ensures both spellings work correctly
- No duplicate data needed in manifests

<Info>
This normalization happens once at manifest load time. Both `defence` and `defense` keys will work in API calls and UI components.
</Info>
