...
Agility is trained passively by moving through the world:

**Movement XP** (`tile-movement.ts`):
- XP is granted in batches to prevent visual spam
- 100 tiles traveled = 50 XP (effectively 1 XP per 2 tiles)
- Both walking and running grant XP
- Teleportation does NOT grant XP
...
- Already-earned XP is preserved
- Maximum loss: ~50 XP worth of progress

<Info>
Agility XP is batched at 100-tile intervals to prevent XP drops from appearing every few seconds. This creates a more pleasant visual experience while maintaining the same effective XP rate.
</Info>

### Prayer XP

Prayer is trained by burying bones or using them on altars:

**Bone Types**:
| Bone | XP (Bury) | XP (Altar) |
|------|-----------|------------|
| Bones | 4.5 | 15.75 |
| Big bones | 15 | 52.5 |
| Dragon bones | 72 | 252 |
...
<Info>
Using bones on an altar grants 3.5x the XP compared to burying them. Altars are found in churches and temples.
</Info>
...
## Agility & Stamina System

Agility is a unique skill that affects stamina regeneration and is trained through movement.

### Movement-Based XP

Agility XP is granted automatically as you traverse the world:

**XP Batching**:
- XP is granted every 100 tiles traveled
- Each batch grants 50 XP (effective rate: 1 XP per 2 tiles)
- Prevents visual spam from constant XP drops
- Both walking and running count toward progress

**XP Rates by Movement Type**:
| Movement | Tiles/Tick | Tiles/Second | XP/Minute | Time to Level 50 |
|----------|------------|--------------|-----------|------------------|
| Walking | 2 | 3.33 | ~100 XP | ~17 hours |
| Running | 4 | 6.67 | ~200 XP | ~8.5 hours |

**Death Penalty**:
- Tiles accumulated toward the next XP grant are reset to 0
- Already-earned XP is preserved
- Maximum loss per death: 99 tiles (~50 XP worth of progress)

<Warning>
Teleportation does NOT grant Agility XP. Only tile-based movement (walking/running) counts.
</Warning>

### Weight-Based Stamina Drain

Inventory weight affects how quickly stamina drains while running:

**Formula**: `Drain Rate = Base Drain × (1 + Weight × 0.005)`

**Drain Rates by Weight**:
| Weight (kg) | Drain Multiplier | Stamina/Second |
|-------------|------------------|----------------|
| 0 kg | 1.0x | 2.0 |
| 10 kg | 1.05x | 2.1 |
| 20 kg | 1.1x | 2.2 |
...
**Weight Calculation**:
- Each item has a `weight` property (in kg)
- Total weight = sum of (item weight × quantity) for all inventory items
- Weight is calculated server-side and synced to client
- Equipment weight is included in total

<Info>
Weight is displayed in the Equipment Panel. Heavier loads drain stamina faster, encouraging players to manage their inventory strategically.
</Info>
...
**Regeneration Rates by Agility Level**:
...
- **Idle**: 4 stamina/second
- **Walking**: 2 stamina/second
- **Running**: -2 stamina/second (drain, modified by weight)

<Info>
High Agility levels significantly improve stamina sustainability. A level 99 Agility player regenerates stamina nearly twice as fast as a level 1 player.
</Info>

### Stamina Mechanics

**Stamina Range**: 0-100 points

**Behavior**:
- Running drains stamina (rate affected by weight)
- Walking regenerates stamina (rate affected by Agility)
- Standing idle regenerates stamina faster (rate affected by Agility)
- When stamina reaches 0, player is forced to walk
- Stamina must regenerate above 1 before running is re-enabled

**Implementation**:
- Stamina is client-side only (visual indicator)
- Weight is server-authoritative and synced to client
- Agility level is server-authoritative and synced to client
...
<Info>
See `manifests/skill-unlocks.json` for the complete list of unlocks for all skills. Click any skill in the Skills Panel to view its unlock progression.
</Info>

### Skill Guide Panel

Players can view detailed unlock progression for any skill:

**How to Access**:
1. Open the Skills Panel (🧠 icon)
2. Click on any skill icon
3. A popup shows all unlocks for that skill

**Features**:
- Shows all unlocks with level requirements
- Visual distinction between unlocked (green ✓) and locked (🔒) items
- Highlights the next unlock you're working toward
- Progress indicator showing unlocks achieved
- Scrollable list for skills with many unlocks

<Info>
The Skill Guide Panel fetches unlock data from the server endpoint `/api/data/skill-unlocks` to ensure server-authoritative progression information.
</Info>

### Skill Guide Panel

Players can click any skill in the Skills Panel to open an OSRS-style guide showing all unlocks for that skill:

**Features**:
- **Click-to-view**: Click any skill icon to open its guide
- **Unlock progression**: Shows all unlocks with level requirements
- **Visual distinction**: Unlocked items shown in green with checkmarks, locked items dimmed with lock icons
- **Next unlock highlight**: The next unlock you're working toward is highlighted in gold
- **Progress tracking**: Shows "X of Y unlocks achieved" at the top
- **Type indicators**: Badges show whether unlocks are items or abilities
- **Scrollable**: Handles skills with many unlocks (Prayer has 29!)

**Implementation**:
- **Server API**: `GET /api/data/skill-unlocks` provides unlock data
- **Client Component**: `SkillGuidePanel.tsx` renders the modal
- **Loading States**: Shows spinner while fetching data
- **Animations**: Smooth fade-in and slide-up transitions

<Info>
The Skill Guide Panel is purely informational - it doesn't affect gameplay, just helps players understand what they can unlock at each level.
</Info>
