...
| Skill | Description | Training Methods | Tool Validation |
|-------|-------------|------------------|-----------------|
| **Woodcutting** | Chop trees | Using hatchet on trees | Manifest-based (prevents pickaxe usage) |
| **Mining** | Mine ore | Using pickaxe on rocks | Manifest-based (prevents hatchet usage) |
| **Fishing** | Catch fish | Using fishing rod at spots | Exact match required (tools not interchangeable) |
...
<Info>
See [Prayer System](/wiki/game-systems/prayer) for complete prayer mechanics, drain formulas, and altar usage.
</Info>
...
**Tick-Based Respawn**: All resource respawns are now handled by `ResourceSystem.processRespawns()` via deterministic tick counting (OSRS-accurate). Legacy `setTimeout`-based respawn has been removed.
...
**Manifest-Based Validation** (PR #1098): The gathering system now uses `tools.json` manifest as the single source of truth for tool-to-skill mappings.

**Problem Solved**: Substring matching allowed pickaxes to cut trees and hatchets to mine rocks because "pickaxe" contains "axe".

**Solution**: Each tool declares its skill explicitly in the manifest:
...
export function itemMatchesToolCategory(
  ...
  // 2. Fishing tools require exact match (not interchangeable)
  if (isExactMatchFishingTool(category)) {
    return lowerItemId === category;
  }

  // 3. Manifest lookup (primary path)
  ...
  // 4. Fallback with cross-skill guards
  if (category === "hatchet") {
  ...
  if (category === "pickaxe") {
    if (lowerItemId.includes("hatchet")) {
      return false; // Reject hatchets for mining
    }
...
```typescript
// Fishing tools are NOT interchangeable (OSRS-accurate)
itemMatchesToolCategory("fishing_rod", "fishing_rod");        // true
itemMatchesToolCategory("fly_fishing_rod", "fishing_rod");    // false
itemMatchesToolCategory("small_fishing_net", "fishing_rod");  // false
```

**Cross-Skill Prevention**:
```typescript
// Pickaxe cannot be used for woodcutting
itemMatchesToolCategory("bronze_pickaxe", "hatchet");  // false

// Hatchet cannot be used for mining
itemMatchesToolCategory("bronze_hatchet", "pickaxe");  // false

// Combat weapons rejected
