...
Players can click any skill in the Skills tab to open a detailed guide showing all unlocks:

**Features**:
- Lists all unlocks with level requirements
- Visual distinction between unlocked (green ✓) and locked (🔒) items
- Highlights the next unlock to work toward
- Shows progress: "X of Y unlocks achieved"
- Type badges (item, ability, area, etc.)
- Scrollable for skills with many unlocks

**Implementation**:
- Component: `packages/client/src/game/panels/SkillGuidePanel.tsx`
- Data source: `/api/data/skill-unlocks` endpoint
- Manifest: `packages/server/world/assets/manifests/skill-unlocks.json`
...
<Info>
The Skill Guide Panel fetches unlock data from the server API to ensure it's always up-to-date with the latest manifest changes.
</Info>
...
- `area` - Access to new zones or locations
