...
| **Prayer** | Temporary combat bonuses | Burying bones, using altars |
...
The Agility skill is trained passively by moving through the world. Unlike traditional RuneScape agility courses, Hyperscape's Agility system rewards exploration and travel.

### Training Mechanics

**XP Gain**:
- 1 XP per 2 tiles traveled (walking or running)
- XP is batched every 100 tiles to prevent visual spam
- Each batch grants 50 XP (equivalent to 1 XP per 2 tiles)
...
// - Running (4 tiles/tick): ~15 seconds between drops
// - Walking (2 tiles/tick): ~30 seconds between drops
```

**Death Penalty**:
- Accumulated tiles toward next XP grant are lost on death
- Already-earned XP is preserved
- Small consequence that adds risk to exploration

**Logout Behavior**:
- Tile counter resets on disconnect
- Maximum ~50 XP can be lost (less than 1 minute of running)

### Stamina System Integration

Agility affects stamina regeneration, while inventory weight affects stamina drain.
...
const weightMultiplier = 1 + (totalWeight * 0.005);
const drainRate = baseDrain * weightMultiplier;
```
...
```typescript
// Base regen: 2/sec walking, 4/sec idle
// Agility bonus: +1% regen per agility level

const agilityMultiplier = 1 + (agilityLevel * 0.01);
const regenRate = baseRegen * agilityMultiplier;
```
...
Players can click any skill in the Skills Panel to view a detailed guide showing all unlocks at each level.

### Features

- **Click-to-View**: Click any skill icon to open its guide
- **Unlock Progression**: Shows all items, abilities, and areas unlocked at each level
- **Visual States**:
  - ✓ Green checkmark for unlocked content (player level ≥ requirement)
  - 🔒 Lock icon for locked content (player level < requirement)
  - ➤ Arrow for next unlock (highlighted with gold border)
- **Progress Tracking**: Shows "X of Y unlocks achieved"
- **Next Unlock Indicator**: Displays how many levels until next unlock
- **Type Badges**: Distinguishes between items and abilities

### UI Components

**SkillGuidePanel** (`packages/client/src/game/panels/SkillGuidePanel.tsx`):
- Portal-rendered modal overlay
...
**Data Source**:
- Server endpoint: `GET /api/data/skill-unlocks`
- Returns all skill unlock definitions from `skill-unlocks.json`
- Client-side caching with loading state
...
const [skillUnlocks, setSkillUnlocks] = useState<Record<string, SkillUnlock[]>>({});

// Fetch unlocks from server
useEffect(() => {
...
**Server Endpoint** (`packages/server/src/startup/routes/data-routes.ts`):
```typescript
