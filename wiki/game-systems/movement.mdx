...
The tile movement system tracks distance traveled for Agility skill XP:

```typescript
// From TileMovementManager (packages/server/src/systems/ServerNetwork/tile-movement.ts)
private tilesTraveledForXP: Map<string, number> = new Map();
...
**XP Batching Design:**
- Prevents visual spam (XP drop every 100 tiles instead of every 2 tiles)
- Preserves overflow (walking 150 tiles = 50 XP + 50 tiles remaining)
- Death penalty: resets tile counter to 0 (lose accumulated progress)
- Logout: tile counter not persisted (resets to 0)

**XP Rates:**
| Movement | Tiles/Tick | Tiles/Second | XP/Minute |
|----------|------------|--------------|-----------|
