...
Hyperscape uses a unified **CollisionMatrix** system for OSRS-accurate tile-based collision. This handles static objects (trees, rocks, stations), entities (players, mobs), and terrain features.

### CollisionMatrix Architecture

The collision system uses **zone-based storage** with bitmask flags for optimal performance:
...
- Lazy allocation (zones created only when needed)
- Memory: ~4MB for 1000×1000 tile world

### Collision Flags

OSRS-accurate bitmask flags for different collision types:
...
// Check for specific blocking
if (world.collision.hasFlags(tileX, tileZ, CollisionMask.BLOCKS_WALK)) {
  // Static object blocking (tree, rock, station)
}

// Register static collision (resources, stations)
world.collision.addFlags(tileX, tileZ, CollisionFlag.BLOCKED);

// Remove collision when object destroyed
world.collision.removeFlags(tileX, tileZ, CollisionFlag.BLOCKED);
```

### Multi-Tile Footprints

Large objects (furnaces, banks, trees) can occupy multiple tiles:

```typescript
// From tilesWithinRangeOfFootprint in TileSystem.ts
...
export function tilesWithinRangeOfFootprint(
  ...
  // Footprint is CENTERED on centerTile
  ...
  for (let dx = 0; dx < footprintWidth; dx++) {
    for (let dz = 0; dz < footprintDepth; dz++) {
      ...
      const distance = Math.max(
        Math.abs(playerTile.x - occupiedTile.x),
        Math.abs(playerTile.z - occupiedTile.z)
      );
      
      if (distance <= rangeTiles && distance > 0) {
        return true; // In range of this tile
      }
    }
  }
...
**Example:** A 2×2 furnace centered at (10, 10) occupies tiles (9,9), (10,9), (9,10), (10,10). Players can interact from any adjacent tile.

### Automatic Footprint Detection

Station footprints are automatically calculated from model bounds at build time:

```typescript
// Build step: extract-model-bounds.ts scans GLB files
// Runtime: StationDataProvider computes footprint = (rawDimensions * modelScale)

// From StationDataProvider.ts
const footprint = stationDataProvider.getFootprint("furnace");
// Returns: { width: 2, depth: 2 } (auto-calculated from model)
```

<Info>
**Build Integration**: Run `bun run extract-bounds` to regenerate `model-bounds.json` after adding new models. This happens automatically during `bun run build` via Turbo caching.
</Info>

## Entity Occupancy

The `EntityOccupancyMap` delegates to `CollisionMatrix` for unified collision storage:
...
export interface IEntityOccupancy {
  ...
  // Update entity position (atomic multi-tile moves)
...
**Integration:** When entities occupy tiles, `EntityOccupancyMap` automatically updates `CollisionMatrix` with `OCCUPIED_PLAYER` or `OCCUPIED_NPC` flags.
