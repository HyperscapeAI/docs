...
The Agility skill trains automatically as players move through the world:

**XP Mechanics**:
- **Rate**: 1 XP per 2 tiles traveled
- **Batching**: XP granted in batches of 50 XP every 100 tiles
- **Tracking**: Server tracks tiles traveled per player in `TileMovementManager`
- **Death Penalty**: Accumulated tiles toward next XP grant are lost on death
- **Logout**: Tile counter resets on disconnect (max ~50 XP lost)
...
Hyperscape features a stamina system that affects running ability. Stamina is client-side only for UI display.

### Stamina Mechanics

```typescript
// From PlayerLocal.ts
public stamina: number = 100;  // Current stamina (0-100)
...
<Warning>
When stamina reaches 0, the player is forced to walk until stamina regenerates above 1.
</Warning>

### Weight Synchronization

Weight is calculated server-side and synced to clients:
...
onPlayerWeightUpdated = (data: { playerId: string; weight: number }) => {
  ...
  if (localPlayer instanceof PlayerLocal) {
