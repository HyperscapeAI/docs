...
Bank withdrawals support three item types with different stacking behavior:

```typescript
// From packages/server/src/systems/ServerNetwork/handlers/bank/core.ts

// STACKING RULES:
// 1. Noted items: Always stackable (1 slot, unlimited quantity)
// 2. Stackable base items: Stack in inventory (arrows, bait, runes)
// 3. Non-stackable items: One slot per item (max 28 per withdrawal)

// Check if base item is stackable
const itemData = getItem(data.itemId);
const isBaseStackable = itemData?.stackable === true;

// Apply withdrawal limits
const withdrawQty = withdrawAsNote || isBaseStackable
  ? data.quantity  // Stackable: no limit
  : Math.min(data.quantity, MAX_INVENTORY_SLOTS); // Non-stackable: 28 max

// Execute transaction with inventory lock
const result = await executeInventoryTransaction(ctx, async () => {
  return executeSecureTransaction(ctx, {
    execute: async (tx) => {
      // ... lock bank and inventory rows
      
      if (withdrawAsNote) {
        // Noted item: Create single stack
        ...
        // Stackable base item: Merge with existing stack or create new
...
<Info>
**Stackable Items**: Arrows, fishing bait, runes, and other stackable items can be withdrawn in unlimited quantities (all fit in 1 inventory slot). Non-stackable items are limited to 28 per withdrawal.
</Info>

### Race Condition Prevention

All bank and store transactions use `executeInventoryTransaction` to prevent race conditions:

```typescript
// From packages/server/src/systems/ServerNetwork/handlers/common/transaction.ts
...
export async function executeInventoryTransaction<T>(
  ...
  // 1. QUEUE: Serialize concurrent operations for same player
  // 2. LOCK: Block pickups and auto-save during transaction
  // 3. FLUSH: Persist pending in-memory changes to DB
  // 4. EXECUTE: Run the transaction (DB is source of truth)
  // 5. RELOAD: Replace in-memory state with DB state
...
- Supports optimistic UI (queue instead of reject)
