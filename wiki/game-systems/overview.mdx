...
- **Trading** — Player-to-player trading with two-screen confirmation
...
**Location:** `server/src/systems/TradingSystem/`

OSRS-accurate player-to-player trading with anti-scam features:

- **Two-Screen Flow**: Offer screen → Confirmation screen
  - Both players must accept on offer screen to move to confirmation
  - Both players must accept on confirmation screen to complete trade
  - Acceptance resets if either offer changes
- **Trade Sessions**: Server-authoritative state management
  - Request → Active → Confirming → Completed/Cancelled
  - Timeout handling (5 minutes for requests, 10 minutes for active trades)
  - Automatic cleanup on disconnect
- **Proximity Checks**: Players must be adjacent (1 tile) to trade
  - `PendingTradeManager` handles automatic pathfinding if out of range
...
- **Atomic Swaps**: Database transactions prevent item duplication
  ...
  - Validation of all items before swap
  - Rollback on failure
- **Anti-Scam Features**:
  - Red flashing indicators when items removed
  - Wealth transfer indicator (value difference)
  - Partner inventory slots display
...
<CardGroup cols={2}>
  ...
  <Card title="Quest System" icon="scroll" href="/wiki/game-systems/quests">
    OSRS-style quests with multi-stage objectives, requirements, rewards, and security features.
  </Card>
  ...
  <Card title="Inventory System" icon="backpack" href="/wiki/game-systems/inventory">
    28-slot inventory management with drag-drop and context menus.
  </Card>
  <Card title="Bank System" icon="vault" href="/wiki/game-systems/bank">
    Unlimited storage with tabs and organization features.
  </Card>
  <Card title="NPC Data" icon="dragon" href="/wiki/data/npcs">
