...
- **Skills** — 12 trainable skills (6 combat, 3 gathering, 3 artisan, 1 support)
- **Inventory** — 28-slot bag with weight tracking
- **Prayer** — Prayer points and active prayer management
...
- **Quest System** — OSRS-style quests with kill tracking, dialogue integration, and rewards
- **Quest Points** — Track quest completion and unlock advanced quests
- **XP Lamps** — Skill selection for quest XP rewards

### Quest System

**Location:** `systems/shared/progression/`

OSRS-style quest progression with objectives and rewards:

- **QuestSystem.ts** — Quest lifecycle management (~1100 lines)
- **Multi-stage quests** — Dialogue, kill, gather, interact objectives
- **Quest points** — Permanent progression tracking
- **Prerequisites** — Quest chains with skill/quest/item requirements
- **Rewards** — Items, XP, and quest points
- **Security** — HMAC kill token validation, rate limiting, audit logging
- **Database** — PostgreSQL persistence with atomic transactions

### Quest System

**Location:** `systems/shared/progression/`

OSRS-style quest progression with multi-stage tracking:

- **Quest Progression** — Multi-stage quests with flexible completion
- **Stage Types** — Dialogue, kill, gather, interact objectives
- **Quest Points** — Character-level quest point tracking
- **Security** — HMAC kill token validation, rate limiting, audit logging
- **Database** — `quest_progress` and `quest_audit_log` tables

### Quest System

**Location:** `systems/shared/progression/`

OSRS-style quest progression with multi-stage quests:

- **QuestSystem.ts** — Quest orchestration and progress tracking
...
<Tip>
  Quest progress is tracked in real-time with counters (e.g., "Kill 15 goblins (7/15)").
</Tip>
...
Hyperscape implements 12 skills with OSRS-accurate mechanics:
...
<CardGroup cols={2}>
  ...
  <Card title="Quest System" icon="scroll" href="/wiki/game-systems/quests">
    Multi-stage quests with kill, gather, and interact objectives. Quest points and rewards.
  </Card>
