...
## Duel Arena Floor Fix (commit b8f56e81)
...
Flat zones are now registered programmatically from `DuelArenaVisualsSystem` for all 8 floor areas:

```typescript
// From DuelArenaVisualsSystem.ts
// Register flat zones for all arena floors
for (let i = 0; i < 6; i++) {
  const arenaCenter = this.getArenaCenter(i);
  terrainSystem.registerFlatZone({
    id: `duel_arena_${i}`,
    centerX: arenaCenter.x,
    centerZ: arenaCenter.z,
    width: ARENA_WIDTH,
    depth: ARENA_LENGTH,
    height: BASE_Y,
    blendRadius: 2.0,
