...
The post-processing system shares the shader pipeline between outline rendering and LUT color grading:

```typescript
// From ClientGraphics.ts
// Route rendering through composer even when post-processing is off
// so entity highlights still work
if (this.postProcessingEnabled || this.outlineEnabled) {
...
// Prevents stale LUT data from leaking into outline-only rendering
...
**Fix Applied (PR #829):**
- **Issue**: When LUT was "none" but hover triggered outline, the full pipeline ran including stale LUT data
