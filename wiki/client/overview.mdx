...
| `EquipmentPanel.tsx` | Equipment slots with stat display and weight tracking |
| `SkillsPanel.tsx` | All 12 skills with XP progress bars and clickable skill guides |
...
The Skill Guide Panel is an OSRS-style popup that shows what you can unlock at each level:

**Features:**
- Click any skill in the Skills Panel to open
- Shows all unlocks with level requirements
- Visual distinction between unlocked (green ✓) and locked (gray 🔒) content
- Highlights next unlock you're working toward (gold ➤)
...
export function SkillGuidePanel({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999]">
      ...
      {/* Panel with OSRS-style theming */}
      <div className="relative bg-[rgba(20,15,10,0.98)] border-2 border-[rgba(139,69,19,0.8)]">
...
**Data Flow:**
```
SkillsPanel → fetch /api/data/skill-unlocks → SkillGuidePanel → render unlocks
```

<Info>
The Skill Guide Panel uses server-authoritative data to ensure unlock information matches the actual game state.
</Info>
