...
On server startup, the server fetches game data manifests from the CDN:
...
- **Development**: Skips fetch if local manifests already exist (allows local development with custom manifests)
- **Caching**: Compares content before writing to avoid unnecessary disk writes
- **Fallback**: Uses existing local manifests if CDN fetch fails

<Info>
This architecture allows updating game content (items, NPCs, etc.) by deploying new manifests to the CDN without requiring server redeployment.
</Info>
