...
The shared package includes a sophisticated procedural terrain generation system with multi-threaded performance:
...
The shared package includes a centralized GPU-instanced particle system for high-performance visual effects:
...
**Architecture:**
- **ParticleManager**: Central router with ownership tracking
- **WaterParticleManager**: GPU-instanced fishing spot effects (splash, bubble, shimmer, ripple)
- **GlowParticleManager**: Instanced glow billboards (altar, fire, torch presets)

**Performance Benefits:**
- Reduces ~150 draw calls to 4 per fishing spot (97% reduction)
- GPU-driven particle animations (parabolic arcs, wobble, twinkle)
...
- **InstancedMeshes**: 4 layers per fishing spot (splash, bubble, shimmer, ripple)
- **TSL NodeMaterials**: GPU-computed animations using Three.js Shading Language
- **InstancedBufferAttributes**: Per-particle data (spotPos, ageLifetime, angleRadius, dynamics)
...
- **Net**: Calm, gentle ripples (4 splash, 3 bubble, 3 shimmer, burst every 5-10s)
- **Bait**: Medium activity (5 splash, 4 bubble, 4 shimmer, burst every 3-7s)
- **Fly**: Active river fishing (8 splash, 5 bubble, 5 shimmer, burst every 2-5s)
...
See [Particle System Documentation](/wiki/engine/particles) for complete API reference and GPU implementation details.
