...
| Category | Systems |
|----------|---------|
| `character/` | Character stats, equipment, skills, inventory, prayer |
| `combat/` | Combat mechanics, damage, death, aggro, animations |
| `death/` | Death handling, respawn, safe zones, wilderness |
| `economy/` | Banks, shops, trading, loot, ground items |
| `entities/` | Entity lifecycle, spawning, resource management |
| `interaction/` | Player-world interactions, cooking, firemaking, smithing |
| `movement/` | Tile-based movement, collision, pathfinding, chase behavior |
| `presentation/` | Visual rendering, particles, music, chat |
| `progression/` | Quest system and progression tracking |
| `tick/` | NPC tick processing and optimization |
| `world/` | Terrain, vegetation, sky, water, environment |
...
<Note>
The `hs-kit` package was removed in January 2026. All UI components are now in `packages/client/src/ui/`.
</Note>
