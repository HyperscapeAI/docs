...
```typescript
// From packages/plugin-hyperia/src/index.ts
actions: [
  // Movement
  moveToAction,
  followEntityAction,
...
**Action Lock:**
```typescript
// From packages/plugin-hyperia/src/managers/autonomous-behavior-manager.ts
...
```mermaid
flowchart TD
    A[ElizaOS Runtime] --> B[Plugin-Hyperia]
    B --> C[Providers]
    B --> D[Actions]
    C --> E[World State]
...
| `@hyperia/shared` | workspace | Core engine |
