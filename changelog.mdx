...
<Update label="Duel Arena, Website, & UI Enhancements" description="January 2026" date="2026-01-28">

### ⚔️ Duel Arena System (PR #685)

Complete PvP dueling system with OSRS-accurate mechanics (15,066 additions):

- **Challenge System**: 30-second challenge requests with distance validation (15 tiles)
- **Rules Negotiation**: 10 combat rule toggles + 11 equipment slot restrictions
...
- **Combat Phase**: 3-2-1 countdown with rule enforcement
...
- **Resolution**: Winner receives all stakes, both players restored to full health
  - 8-tick death animation delay (4.8 seconds)
  - Teleport to duel arena lobby (different spawns for winner/loser)
  - Comprehensive audit logging
- **Client UI**: Complete duel interface with 8 components
...
- **Server Architecture**: SOLID design with 4 manager classes
...
- **Testing**: 1,755 lines of comprehensive unit tests
...
- **Landing Page**: Redesigned hero with responsive layout and staggered animations
  - Hero component with logo, tagline, and CTA button
  - Features section with larger sizing and better alignment
  - CTA section with banner image and gradient overlay
  - Background component for per-page customizable backgrounds
- **$GOLD Token Page**: New `/gold` route with scroll-style design
  - Two-column hero layout with token image
  - Token details with contract address (copy-to-clipboard)
  - Features grid explaining token utility
  - How it works section
  - CTA section with gold-themed banner
...
- **RS3-Style Keyboard Shortcuts**: Expanded shortcuts with modifier support
...
- **Mobile Touch Support**: Long-press drag-and-drop for mobile
  - TouchSensor added to dnd-kit (250ms long-press activation)
  - Long-press terrain for context menu (500ms)
  - Works on inventory, equipment, skills, prayer panels
- **Panel Size Increases**: ~30% larger panels for better readability
...
- **Edit Mode Refactor**: Extracted keyboard handling to prevent duplicate listeners
  - New `useEditModeKeyboard` hook (single instantiation)
  - Hold L for 1 second to unlock, Escape to lock
  - Progress bar shows hold duration
  - Race condition fix in animation loop
- **Delete Zone**: Drag panels to delete zone to remove them
  - Visual feedback on hover
  - Trash icon with label
  - Only visible when dragging windows
- **Chat Message Filtering**: Proper tab filtering (all/game/clan/private)
  - Extended message types: trade, private, clan, guild
  - Color-coded messages (pink for private, green for clan)
  - Context-aware empty state messages
- **Menu Bar Layout**: Responsive grid algorithm
  - Automatically reflows between 1-5 rows
  - Buttons scale to fit container (20-32px)
  - Viewport edge snapping on layout changes
- **Minimap Fill**: Uses larger dimension to always fill container
- **Dialogue Panel Fix**: Removed redundant ModalWindow wrapper
- **Equipment Panel**: Removed character silhouette for cleaner appearance
- **Theme Colors**: New `panelPrimary` and `panelSecondary` colors for better contrast
- **Border Radius**: Standardized to squared corners (borderRadius: 0) for OSRS style
- **Type Safety**: Proper event payload types (no more `unknown` assertions)
- **Performance**: Memoized style objects in duel panels

### 🐛 Bug Fixes

- **Inventory Cross-Tab**: Fixed inventory updates applying to wrong players
- **Bank Close Event**: Standardized to `bankClose` (was `bank_close`)
- **Console Logs**: Removed debug logs from InteractionRouter
- **Unused Imports**: Removed unused `useMemo` import
- **Death Animation**: Fixed remote player death animation race condition
- **PvP Combat Rotation**: Improved player rotation during duel combat
- **Arena Wall Collision**: Aligned collision with visual walls
- **Duel Inventory Isolation**: Fixed inventory isolation between duel tabs
- **Movement Blocking**: Resolved movement bugs after countdown and death

### 📚 Documentation

- **New Pages**: Duel Arena system documentation, Website package documentation, UI improvements guide
- **Updated**: Game systems overview, packages overview, architecture, development guide, combat system, death system
- **Navigation**: Added duel-arena and ui-improvements to docs navigation
