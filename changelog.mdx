...
<Update label=\"Agility Skill & Skill Guide Panel\" description=\"January 2026\" date=\"2026-01-21\">\n\n### 🏃 Agility Skill System\n\nComplete implementation of the Agility skill with movement-based XP and stamina mechanics:\n\n- **Movement XP**: Gain 1 XP per 2 tiles traveled (walking or running)\n  - XP batched at 100 tiles (50 XP) to prevent visual spam\n  - XP drops every ~15 seconds running, ~30 seconds walking\n  - Death penalty: lose accumulated tiles toward next grant (max ~50 XP)\n  - Logout resets tile counter (not persisted)\n- **Weight-Based Stamina Drain**: Inventory weight affects stamina consumption\n  - Formula: `Drain = Base × (1 + Weight × 0.005)`\n  - 0 kg: 2.0/sec, 50 kg: 2.5/sec, 100 kg: 3.0/sec\n  - Weight synced from server via `PLAYER_WEIGHT_CHANGED` event\n  - Equipment panel displays server-calculated total weight\n- **Agility-Based Stamina Regen**: Higher levels regenerate stamina faster\n  - Formula: `Regen = Base × (1 + Agility Level × 0.01)`\n  - Level 1: 4.04/sec idle, Level 99: 7.96/sec idle\n  - Applies to both walking and idle regeneration\n  - Nearly doubles regen rate at level 99\n- **Database**: New `agilityLevel` and `agilityXp` columns with migration\n- **Testing**: 868 lines of comprehensive tests\n  - `AgilitySystem.test.ts`: Formula validation and edge cases\n  - `AgilityPersistence.test.ts`: Database persistence and death mechanics\n\n### 📊 Skill Guide Panel\n\nOSRS-style skill guide interface showing unlock progression:\n\n- **Click-to-View**: Click any skill in Skills Panel to open its guide\n- **Unlock Display**: Shows all unlocks with level requirements\n  - Green checkmark for unlocked items\n  - Lock icon for locked items\n  - Next unlock highlighted with progress indicator\n- **Type Badges**: Visual indicators for unlock types (item, ability, area, quest, activity)\n- **Scrollable**: Handles skills with many unlocks (Prayer has 29)\n- **Loading State**: Spinner while fetching unlock data from server\n- **Server API**: New `/api/data/skill-unlocks` endpoint\n- **Animations**: Smooth fade-in and slide-up transitions\n- **Keyboard Support**: ESC key to close, click outside to dismiss\n\n### 🚀 Railway Deployment Architecture\n\nProduction deployment infrastructure with split frontend/backend:\n\n- **Frontend**: Cloudflare Pages (hyperscape.pages.dev)\n  - Automatic deployments from main branch\n  - Preview deployments for pull requests\n  - Global CDN distribution\n- **Backend**: Railway (hyperscape-production.up.railway.app)\n  - Nixpacks builder configuration\n  - Automatic deployments via GitHub Actions\n  - PostgreSQL database included\n  - Manifest fetching from CDN at startup\n- **Assets**: Cloudflare R2 CDN\n  - Manifests, 3D models, textures, audio\n  - Automatic uploads via GitHub Actions\n  - 5-minute cache headers for manifests\n- **CORS Configuration**: Cloudflare Pages domains added to allowlist\n  - `hyperscape.pages.dev` (production)\n  - `*.hyperscape.pages.dev` (preview deployments)\n\n### 🔧 CDN Manifest Fetching\n\nServer now fetches manifests from CDN at startup instead of requiring Git LFS:\n\n- **Startup Fetch**: `fetchManifestsFromCDN()` downloads manifests on server start\n- **Development Skip**: Skips CDN fetch if local manifests exist\n- **CI/Production**: Always fetches from CDN (no Git LFS needed)\n- **Conditional Asset Routes**: `/assets/` route only registered if local assets exist\n- **Manifest Cache Headers**: 5-minute cache for manifest JSON files\n- **Environment Detection**: Skips asset download in CI/Railway via `CI` env var\n\n### 🐛 Bug Fixes\n\n- **Build Process**: Split build commands for better Railway debugging\n- **Frontend Serving**: Fixed client build copying to server public directory\n- **Index.html Verification**: Added checks to ensure frontend build succeeded\n- **CORS**: Added Cloudflare Pages domains to CORS allowlist\n- **Lockfile**: Updated bun.lock to sync with package.json dependencies\n- **Defence Spelling**: Normalized British \"defence\" to American \"defense\" in skill unlocks loader\n\n</Update>
