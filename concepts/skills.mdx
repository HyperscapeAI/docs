...
Hyperscape has **13 skills** total:
...
| **Agility** | Stamina regeneration (trained by moving) |
...
Click any skill in the Skills panel to view its Skill Guide - an OSRS-style interface showing all unlocks for that skill.

**Features**:
- Shows all level-based unlocks with requirements
- Green checkmarks (✓) for unlocked content
- Lock icons (🔒) for locked content
- Highlights your next unlock with progress indicator
- Type badges distinguish items from abilities
- Scrollable for skills with many unlocks (Prayer has 29!)

**How to Use**:
1. Open the Skills panel (default keybind: Skills icon in sidebar)
2. Click on any skill icon
3. View all unlocks for that skill
4. Close with ESC, X button, or click outside

<Info>
The Skill Guide fetches data from `/api/data/skill-unlocks` to ensure server-authoritative unlock information.
</Info>
