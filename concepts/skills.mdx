---
title: "Skills System"
description: "Skill progression, XP formulas, and resource gathering"
icon: "trophy"
---

## Overview

Hyperscape features a RuneScape-inspired skill system with leveling, XP gain, and level requirements for equipment and activities.

## Available Skills

Hyperscape has **11 skills** total:

### Combat Skills

| Skill | Purpose |
|-------|---------|
| **Attack** | Melee accuracy, weapon requirements |
| **Strength** | Melee damage |
| **Defense** | Damage reduction, armor requirements |
| **Constitution** | Health points |
| **Ranged** | Ranged combat accuracy and damage |

### Gathering Skills

| Skill | Activity |
|-------|----------|
| **Woodcutting** | Chop trees for logs |
| **Fishing** | Catch fish at water spots |
| **Mining** | Mine ore from rocks |

### Artisan Skills

| Skill | Activity |
|-------|----------|
| **Firemaking** | Create fires from logs |
| **Cooking** | Prepare food from raw fish |
| **Smithing** | Smelt ores into bars and smith equipment |

<Info>
  All skills use OSRS-accurate XP formulas and tick-based timing (600ms per tick).
</Info>

## XP and Leveling

### XP Formula

Hyperscape uses RuneScape's XP curve:

```
XP for Level N = floor(sum(floor(L + 300 * 2^(L/7))) / 4) for L = 1 to N-1
```

### Level Milestones

| Level | Total XP |
|-------|----------|
| 10 | 1,154 |
| 20 | 4,470 |
| 30 | 13,363 |
| 50 | 101,333 |
| 70 | 737,627 |
| 99 | 13,034,431 |

## Resource Gathering

### Woodcutting

Requirements:
- Hatchet in inventory or equipped
- Click tree to chop

Success rate improves with level. Higher-level trees require higher Woodcutting levels.

| Tree | Level | XP |
|------|-------|-----|
| Normal | 1 | 25 |
| Oak | 15 | 37.5 |
| Willow | 30 | 67.5 |
| Yew | 60 | 175 |
| Magic | 75 | 250 |

### Mining

Requirements:
- Pickaxe in inventory or equipped
- Click ore rock to mine

Success rate improves with level and pickaxe tier.

| Ore | Level | XP |
|-----|-------|-----|
| Copper/Tin | 1 | 17.5 |
| Iron | 15 | 35 |
| Coal | 30 | 50 |
| Mithril | 55 | 80 |
| Adamantite | 70 | 95 |
| Runite | 85 | 125 |

### Fishing

Requirements:
- Fishing rod in inventory
- Click fishing spot

| Fish | Level | XP |
|------|-------|-----|
| Shrimp | 1 | 10 |
| Trout | 20 | 50 |
| Lobster | 40 | 90 |
| Swordfish | 50 | 100 |
| Shark | 76 | 110 |

## Artisan Skills

### Firemaking

Use tinderbox on logs in inventory to create a fire.

| Log | Level | XP |
|-----|-------|-----|
| Normal | 1 | 40 |
| Oak | 15 | 60 |
| Willow | 30 | 90 |
| Yew | 60 | 202.5 |
| Magic | 75 | 303.8 |

### Cooking

Use raw fish on a fire or range to cook.

| Food | Level | XP | Heals |
|------|-------|-----|-------|
| Shrimp | 1 | 30 | 3 HP |
| Trout | 15 | 70 | 7 HP |
| Lobster | 40 | 120 | 12 HP |
| Swordfish | 45 | 140 | 14 HP |
| Shark | 80 | 210 | 20 HP |

<Warning>
  Cooking can fail at low levels, burning the food. Ranges reduce burn chance compared to fires.
</Warning>

### Smithing

Smelt ores into bars at furnaces, then smith bars into equipment at anvils.

**Smelting** (at furnace):
| Bar | Level | Ores Required | XP |
|-----|-------|---------------|-----|
| Bronze | 1 | 1 Copper + 1 Tin | 6.2 |
| Iron | 15 | 1 Iron (50% success) | 12.5 |
| Steel | 30 | 1 Iron + 2 Coal | 17.5 |
| Mithril | 50 | 1 Mithril + 4 Coal | 30 |
| Adamant | 70 | 1 Adamantite + 6 Coal | 37.5 |
| Rune | 85 | 1 Runite + 8 Coal | 50 |

**Smithing** (at anvil, requires hammer):
| Item | Level | Bars | XP |
|------|-------|------|-----|
| Bronze dagger | 1 | 1 | 12.5 |
| Bronze sword | 4 | 1 | 12.5 |
| Bronze platebody | 18 | 5 | 62.5 |
| Iron sword | 19 | 1 | 25 |
| Steel sword | 34 | 1 | 37.5 |
| Mithril sword | 54 | 1 | 50 |
| Adamant sword | 74 | 1 | 62.5 |
| Rune sword | 89 | 1 | 75 |

<Info>
  Smithing recipes are organized by category (weapons, armor, tools) and defined in `manifests/recipes/smithing.json`.
</Info>

## Equipment Level Requirements

Equipment and activities have level gates:

| Tier | Attack/Defense | Ranged |
|------|----------------|--------|
| Bronze | 1 | 1 |
| Iron | 5 | - |
| Steel | 10 | 10 |
| Mithril | 20 | 20 |
| Adamant | 30 | - |
| Rune | 40 | - |
| Dragon | 60 | - |

## Skill Data Files

Skill configuration is in `packages/shared/src/data/`:

| File | Purpose |
|------|---------|
| `equipment-requirements.json` | Level requirements for equipment |
| `equipment-stats.json` | Stat bonuses for equipment |

## Default Skill Values

Skills start at level 1 with 0 XP, except Constitution which starts at level 10:

```typescript
// From packages/shared/src/entities/player/PlayerEntity.ts (lines 111-123)
playerData.skills = {
  attack: { level: 1, xp: 0 },
  strength: { level: 1, xp: 0 },
  defense: { level: 1, xp: 0 },
  constitution: { level: 10, xp: 1154 },  // Starts at level 10
  ranged: { level: 1, xp: 0 },
  woodcutting: { level: 1, xp: 0 },
  fishing: { level: 1, xp: 0 },
  firemaking: { level: 1, xp: 0 },
  cooking: { level: 1, xp: 0 },
};
```

<Info>
  Constitution starts at level 10 (1,154 XP) like RuneScape's Hitpoints.
  Player health equals their Constitution level.
</Info>
