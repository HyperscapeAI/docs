...
// │   ├── BankEntity (bank storage)
// │   ├── AnvilEntity (smithing station)
// │   ├── FurnaceEntity (smelting station)
// │   ├── RangeEntity (cooking station)
// │   ├── AltarEntity (prayer restoration)
// │   ├── RunecraftingAltarEntity (rune crafting)
// │   ├── FireEntity (player-lit fires for cooking)
// │   └── StarterChestEntity (tutorial items)
...
// │   └── MobEntity (enemies)
// └── HeadstoneEntity (player death markers with loot)
```
...
this.addComponent("stats", { attack, strength, defense, constitution, ranged, woodcutting, fishing, mining, firemaking, cooking, smithing });
```
