...
The server fetches manifests from the CDN at startup to ensure content is always up-to-date:
...
**Fetch Behavior:**
- **Production**: Always fetch from CDN, cache locally
- **Development**: Skip if local manifests exist (allows local modifications)
- **CI/Test**: Skip if `SKIP_MANIFESTS=true` or `NODE_ENV=test`

**Cache Headers:**
Manifests use `Cache-Control: public, max-age=300, must-revalidate` (5 minute cache) to balance freshness with performance.
