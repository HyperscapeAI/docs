...
The `quests.json` manifest defines OSRS-style quests with multi-stage progression:
...
**Stage Types**:
- `dialogue` — Talk to NPC to progress
- `kill` — Kill specific number of mobs
- `gather` — Collect items through gathering skills
- `interact` — Complete skill actions (cooking, smithing, etc.)

**Quest Integration**:
- Quests start through NPC dialogue with `questOverrides`
- Dialogue nodes can have `effect: "startQuest:quest_id"` or `effect: "completeQuest:quest_id"`
- Progress tracked automatically via game events (NPC_DIED, INVENTORY_ITEM_ADDED, etc.)
- Quest points stored in `characters.questPoints` column

<Info>
Quest progress is tracked in the `quest_progress` database table with flexible JSON progress data. See [Quest System](/wiki/game-systems/quests) for full documentation.
</Info>
