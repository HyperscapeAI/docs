...
The `quests.json` manifest defines OSRS-style quests with multi-stage objectives, requirements, and rewards:
...
**Quest Properties:**
- `id` — Unique quest identifier (snake_case, max 64 chars)
- `name` — Display name in quest journal
...
- `requirements` — Prerequisites (quests, skills, items)
...
**Stage Types:**
- `dialogue` — Talk to NPC (no additional properties)
- `kill` — Kill mobs (`target`: mob type, `count`: number to kill)
- `gather` — Gather items (`target`: item ID, `count`: number to gather)
- `interact` — Interact with objects (`target`: entity type, `count`: number of interactions)

**Quest Status:**
- `not_started` — Quest not yet accepted
- `in_progress` — Quest active, objectives incomplete
- `ready_to_complete` — All objectives done (derived status, not stored)
- `completed` — Quest finished

<Info>
Quests integrate with the dialogue system via `questOverrides` in NPC definitions. See [Quest System](/wiki/game-systems/quests) for complete documentation.
</Info>
...
9. **Quests** (`quests.json`) - Loaded by QuestSystem separately
