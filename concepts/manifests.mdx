...
The `quests.json` manifest defines multi-stage quests with requirements and rewards:
...
- `kill` — Kill specific NPCs (with HMAC validation)
- `gather` — Collect items via gathering skills
- `interact` — Use objects/stations (fires, cooking, smithing)

**Security Features**:
- Quest IDs validated with strict pattern (`/^[a-z][a-z0-9_]{0,63}$/`)
- HMAC-SHA256 kill tokens prevent event spoofing
- Rate limiting on quest network handlers
- Audit logging for all quest state changes

<Info>
See [Quest System](/wiki/game-systems/quests) for complete documentation on quest mechanics and API reference.
</Info>
