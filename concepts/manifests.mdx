...
**Smart caching:**
- Server compares fetched content with local cache
- Only writes if content changed (minimizes disk I/O)
- Logs update status: "X fetched, Y updated, Z failed"

<Info>
In development, if local manifests exist, CDN fetch is skipped to allow working with local asset modifications.
</Info>
...
2. **CDN** (client-side): Fetches from `http://localhost:8080/manifests/` or production CDN
...
### CDN Manifest Fetching (Production)
...
3. Downloads all manifest JSON files from CDN
...
**Benefits:**
- Manifests can be updated without redeploying the server
- Smaller Docker images (no bundled JSON files)
- Faster deployments (no asset cloning in CI)
- Single source of truth (CDN)

**Fallback behavior:**
If CDN fetch fails and no local manifests exist:
- In test/CI: Continues with warning (minimal mode)
- In production: Throws error (prevents starting with stale data)

**Cache invalidation:**
The server compares fetched content with existing files and only writes if content changed. This avoids unnecessary disk I/O and preserves file timestamps.
...
- `requirements` — Prerequisites to start the quest
  ...
  - `skills` — Object mapping skill names to required levels
  - `items` — Array of required item IDs
- `startNpc` — NPC ID to start the quest
- `stages` — Array of quest stages in order
  - `id` — Unique stage identifier
  - `type` — Stage type: `dialogue`, `kill`, `gather`, `deliver`
  - `description` — Stage objective description
  - `target` — Target NPC/mob/item ID (for kill/gather stages)
  - `count` — Required count (for kill/gather stages)
- `onStart` — Actions triggered when quest starts
  - `items` — Items given to player
  - `dialogue` — Dialogue key to display
- `rewards` — Quest completion rewards
  - `questPoints` — Quest points awarded
  - `items` — Items given as rewards
  - `xp` — Object mapping skill names to XP amounts

**Stage Types**:
- **dialogue** — Talk to an NPC
- **kill** — Defeat a specific number of mobs
- **gather** — Collect a specific number of items
- **deliver** — Bring items to an NPC

<Info>
Quest progress is tracked server-side in the database. The quest system supports branching dialogues and conditional stage progression.
</Info>
