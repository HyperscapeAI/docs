...
In production and CI environments, the server **fetches manifests from CDN at startup** instead of bundling them in the deployment:
...
**Behavior**:
- **Production/CI**: Always fetches manifests from `PUBLIC_CDN_URL`
- **Development**: Skips fetch if local manifests already exist (allows local development)
- **Fallback**: Uses existing local manifests if CDN fetch fails
...
<Info>
This approach reduces deployment size and ensures manifests stay synchronized with CDN assets without requiring full asset downloads.
</Info>
